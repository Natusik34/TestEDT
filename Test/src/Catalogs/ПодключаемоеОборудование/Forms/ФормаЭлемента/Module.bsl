...
#КонецЕсли 
	...
	Иначе                    
		ДоступнаЗагрузкаССайтаИТС = Объект.ДрайверОборудования.СпособПодключения = Перечисления.СпособПодключенияДрайвера.ИзИнформационнойБазы И НЕ Объект.ДрайверОборудования.СнятСПоддержки;
		Элементы.ФормаЗагрузитьССайтаИТС.Видимость = ДоступнаЗагрузкаССайтаИТС И ОбщегоНазначенияБПО.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ПолучениеВнешнихКомпонент");
	КонецЕсли;
