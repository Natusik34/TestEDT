...
Процедура СтраницыПриСменеСтраницы(Элемент, ТекущаяСтраница) 
	...
	Если Не ТипЗнч(ТекущаяСтраница) = Тип("ГруппаФормы") Тогда
		Возврат
	КонецЕсли;
...
Процедура ЗагрузитьРезультатИзФайлаВТабличнуюЧасть(РезультатЗагрузки, ТаблицаСопоставленияДанных, 
	...
	Попытка
		...
		Для Каждого СтрокаТаблицы Из ТаблицаСопоставленияДанных Цикл
			...
			Если ИмяТабличнойЧастиЗапасы = "Запасы" И ПустаяСтрока(СтрокаТаблицы.Сумма_ВходящиеДанные) 
				И ПустаяСтрока(СтрокаТаблицы.Цена_ВходящиеДанные) Тогда 
			...
			Если ЗагрузкаВПриложениеВозможна Тогда
				...
				Если Объект[ИмяТабличнойЧастиПокупатели].Количество() = 0 Тогда
						...
						Если НЕ ЗначениеЗаполнено(НоваяСтрока.Покупатель) Тогда
							...
							Если ПараметрыШапки.Свойство("Плательщик") И ПараметрыШапки["Плательщик"] <> Неопределено Тогда
								...
								НоваяСтрока.Покупатель = ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.НайтиКонтрагентаПоИНН_КПП(
				...
				Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТаблицы, "Доплата_ВходящиеДанные")
					...
					Если ЗначениеЗаполнено(СуммаЗаСчет_Ozon) И ЗначениеЗаполнено(СуммаРеализации_Ozon) Тогда 
						СуммаСтроки_СДоплатой = СуммаЗаСчет_Ozon + СуммаРеализации_Ozon;
					КонецЕсли;
					...
					Если ИмяТабличнойЧастиЗапасы = "ЗапасыВозвраты" И ПустаяСтрока(СтрокаТаблицы.Сумма_ВходящиеДанные)
						...
						СуммаСтроки_СДоплатой = СуммаВозвращено_Ozon + СуммаЗаСчет_Ozon;	
					КонецЕсли;
				КонецЕсли;
