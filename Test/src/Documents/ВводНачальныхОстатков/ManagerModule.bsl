...
// Во всех документах с разделом учета = Запасы устанавливается флаг ЗакупкиКУДиРОтдельнымДокументом = Истина,
// если уже есть документ ЗакупкиТоваровДляУСН
// 
// Параметры:
...
Процедура ЗаполнитьПризнакЗакупкиКУДиРОтдельнымДокументом(ПараметрыОбработчика) Экспорт
	...
	Пока РезультатЗапроса.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр(
				"ru = 'Не удалось обработать ввод начальных остатков: %1 по причине:
				|%2'"), РезультатЗапроса.ВводОстатков, ОбработкаОшибок.ПодробноеПредставлениеОшибки(
				ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
				УровеньЖурналаРегистрации.Предупреждение, Метаданные.Документы.ВводНачальныхОстатков,
				РезультатЗапроса.ВводОстатков, ТекстСообщения);
		КонецПопытки;
	КонецЦикла;
	...
	Если ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаполнитьПризнакЗакупкиКУДиРОтдельнымДокументом не удалось обработать некоторые документы ВводНачальныхОстатков (пропущены): %1'"),
			ПроблемныхОбъектов);
