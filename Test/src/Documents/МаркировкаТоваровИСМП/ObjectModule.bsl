...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ОтчетПроизводственнойЛинии Тогда
		...
		Если ТребуетсяУказаниеСертификации(ВидПродукции, ОперацииМаркировки)
			И Не ЗначениеЗаполнено(ВидДокументаСертификации) Тогда
	...
	Иначе
		...
		ИначеЕсли ИнтеграцияИСПовтИсп.ЭтоПродукцияИСМП(ВидПродукции) Тогда
			...
			ИначеЕсли Операция = Перечисления.ВидыОперацийИСМП.ВводВОборотПроизводствоРФ
				...
				Если ВидПродукции = Перечисления.ВидыПродукцииИС.УпакованнаяВода Тогда
					ПроверитьЗаполнениеСертификацииТоваров(Отказ);
				КонецЕсли;
...
Функция ТребуетсяУказаниеСертификации(ВидПродукции, ОперацииМаркировки)
	...
	Если ЭтоПиво И ОперацииМаркировки.ЭтоТрансграничнаяТорговля Тогда
		Возврат Истина;
	ИначеЕсли ЭтоУпакованнаяВода И ОперацииМаркировки.ЭтоВводВОборот
		И Не ОперацииМаркировки.ЭтоИмпортСФТС Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеСертификацииТоваров(Отказ)
	...
	Для Каждого СтрокаТовары Из Товары Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаТовары.ВидДокументаСертификации) Тогда
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'В строке %1 табличной части Товары не заполнено поле %2'"),
				СтрокаТовары.НомерСтроки,
				НСтр("ru = 'Сертификация'"));
...
Функция ТекстОшибкиВСтрокеТабличнойЧастиТоварыНеЗаполненоПоле(НомерСтроки, ИмяПоля)
	
	Возврат СтрШаблон(
				НСтр(
					"ru = 'В строке %1 табличной части Товары не заполнено поле %2'"),
				НомерСтроки,
				ИмяПоля);
