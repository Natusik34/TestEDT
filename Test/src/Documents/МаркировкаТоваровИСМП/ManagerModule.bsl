...
Процедура ПроверитьЗаполнениеСертификацииШапки(СообщениеJSON, Шапка)
	Если Не Шапка.ОтчетПроизводственнойЛинии Тогда
		Возврат;
	КонецЕсли;
	Если Не ЗначениеЗаполнено(Шапка.ВидДокументаСертификации) Тогда
		ИнтеграцияИСКлиентСервер.ДобавитьТекстОшибки(
			СообщениеJSON,
			НСтр("ru = 'Не заполнено поле ""ВидДокументаСертификации""'"));
	КонецЕсли;
КонецПроцедуры

Процедура ПроверитьЗаполнениеСертификации(СообщениеJSON, ВидДокументаСертификации, Номенклатура)
	Если Не ЗначениеЗаполнено(ВидДокументаСертификации) Тогда
		ИнтеграцияИСКлиентСервер.ДобавитьТекстОшибки(
			СообщениеJSON,
			СтрШаблон(НСтр("ru = 'Не заполнено поле ""Сертификация"" для номенклатуры %1.'"), Номенклатура));
	КонецЕсли;
КонецПроцедуры
...
Функция ЗаполнитьДанныеСтрокиВводВОборот(СообщениеJSON, ВидПродукции, СтрокаИсточникаДанных, ИсточникДанныхТовары, Настройки)
	...
	Если СтрокаИсточникаДанных.ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская
		...
		Если СтрокаИсточникаДанных.ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская Тогда
			// Сертификация
			Если Настройки.ПроверятьУказаниеСертификации Тогда
				ПроверитьЗаполнениеСертификации(СообщениеJSON, СтрокаТЧТовары.ВидДокументаСертификации, Отбор.Номенклатура);
			КонецЕсли;
...
Функция НастройкиДобавляемыхПолейСведенийОТоварах(ИмяРеквизитаТовары = "products", ИмяРеквизитаКодМаркировки = "uit_code", ИмяРеквизитаКодУпаковки = "uitu_code")
	...
	Результат.Вставить("ПроверятьУказаниеСертификации", Ложь);
...
#Область ТелоДокумента
	...
	Если Шапка.Операция = Перечисления.ВидыОперацийИСМП.ВводВОборотПроизводствоРФ Тогда
		...
		Если Шапка.ОтчетПроизводственнойЛинии Тогда
			...
			КонецЕсли;
			Если ЭтоУпакованнаяВода Тогда
				ПроверитьЗаполнениеСертификацииШапки(СообщениеJSON, Шапка);
			КонецЕсли;
		КонецЕсли;
	...
	ИначеЕсли Шапка.Операция = Перечисления.ВидыОперацийИСМП.ВводВОборотПроизводствоВнеЕАЭС Тогда
		...
		Настройки.ЗаполнятьВложенныеКоды                   = КодыАгрегированы;
		Настройки.ОтчетПроизводственнойЛинии               = Шапка.ОтчетПроизводственнойЛинии;
		Настройки.ПроверятьУказаниеСертификации            = ЭтоУпакованнаяВода;
	...
	ИначеЕсли Шапка.Операция = Перечисления.ВидыОперацийИСМП.ВводВОборотТрансграничнаяТорговля Тогда
		...
		Настройки.УказыватьИдентификаторПроисхожденияВЕТИС = Истина;
		Настройки.ЗаполнятьВложенныеКоды                   = КодыАгрегированы;
		Настройки.ПроверятьУказаниеСертификации            = ЭтоУпакованнаяВода Или ЭтоПиво;
		
		Если Шапка.ОтчетПроизводственнойЛинии
			И (ЭтоПиво Или ЭтоУпакованнаяВода) Тогда
			ПроверитьЗаполнениеСертификацииШапки(СообщениеJSON, Шапка);
		КонецЕсли;
