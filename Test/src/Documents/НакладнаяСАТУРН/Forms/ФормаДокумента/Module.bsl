...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	ИнтеграцияСАТУРН.ЗаполнитьСопоставленныеТовары(Объект.Товары, НоменклатураДляВыбора);
...
Процедура ТоварыПартияОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ИнтеграцияСАТУРНКлиентСервер.ЭтоВыборПартииСАТУРН(ВыбранноеЗначение) Тогда
		...
		ТекущиеДанные = Элементы.Товары.ТекущиеДанные;
		ЗаполняемыеПоля = СтрРазделить("Номенклатура,Характеристика,Серия,ПАТ", ",");
		ТекущиеДанные.Партия = ВыбранноеЗначение.Партия;
		
		Для Каждого Ключ Из ЗаполняемыеПоля Цикл 
			Если Не ЗначениеЗаполнено(ТекущиеДанные[Ключ])
				И ВыбранноеЗначение.Свойство(Ключ) Тогда
				ТекущиеДанные[Ключ] = ВыбранноеЗначение[Ключ];
			КонецЕсли;
		КонецЦикла;
...
Процедура УстановитьУсловноеОформление()
	...
	ИнтеграцияСАТУРН.УстановитьУсловноеОформлениеПоляНоменклатура(ЭтотОбъект);
	ИнтеграцияСАТУРН.УстановитьУсловноеОформлениеПоляХарактеристика(ЭтотОбъект);
...
Процедура ОбновитьСтатусСАТУРН()
	...
	РедактированиеТоваров = СтатусСАТУРН = Перечисления.СтатусыОбработкиНакладнойСАТУРН.ВПути
	                                       И ГрузополучательСопоставленСОрганизацией
	                                       И Объект.ВходящаяНакладная;
...
Процедура ОбновитьПодсказкиВвода(Форма)
	...
	Если ЗначениеЗаполнено(Форма.Объект.ГрузополучательОрганизацияСАТУРН)
		...
		Форма.Элементы.ГрузополучательОрганизацияСАТУРН.ОтметкаНезаполненного = ЗначениеЗаполнено(Форма.ГрузополучательИзОснованияДляОтбораОрганизацииСАТУРН);
	Иначе
...
Процедура ЗаполнитьОтборПартийПоСтрокеДокумента(ПараметрыФормы)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.Номенклатура) Тогда
		...
		ПараметрыФормы.Упаковка       = ТекущиеДанные.Упаковка;
