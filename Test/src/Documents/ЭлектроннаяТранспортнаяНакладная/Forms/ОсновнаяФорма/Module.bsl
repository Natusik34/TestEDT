...
&НаКлиенте
Процедура НадписьМПОткрытьНажатие(Элемент)
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("УчетнаяЗаписьЭДО", Объект.ИдентификаторПеревозчика);
	ПараметрыФормы.Вставить("Организация", Объект.Организация);
	ПараметрыФормы.Вставить("Идентификатор", МПИдентификаторЗаписи);
...
&НаКлиенте
Процедура НадписьМППозвонитьНажатие(Элемент)
	
	ПерейтиПоНавигационнойСсылке("tel:" + МПТелефон);

КонецПроцедуры

&НаКлиенте
Процедура КомандаВыбратьМПНажатие(Элемент)
	
	ПараметрыДействийМП = СервисВзаимодействияМПЭПДКлиент.ПараметрыДействийДляМобильногоПриложения(
				Объект.Ссылка,
				МПИдентификатор,
				МПИдентификаторЗаписи);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли СтрНачинаетсяС(ИмяСобытия, "Запись_") И Источник <> Неопределено Тогда
		...
		КонецЕсли;
	ИначеЕсли ИмяСобытия = "МПОтвязаноОтДокумента" И Источник = Объект.Ссылка Тогда
		МПИдентификатор = Неопределено;	
		МПИдентификаторЗаписи = Неопределено;
		МПНаименование = Неопределено;
		МПТелефон = Неопределено;
		МПФото = Неопределено;
	...
	ИначеЕсли ИмяСобытия = "МППривязаноКДокументу" И Источник = Объект.Ссылка Тогда
		Параметр.Свойство("МПИдентификатор", МПИдентификатор);	
		Параметр.Свойство("МПИдентификаторЗаписи", МПИдентификаторЗаписи);
		Параметр.Свойство("МПНаименование", МПНаименование);
		Параметр.Свойство("МПТелефон", МПТелефон);
		Параметр.Свойство("МПФото", МПФото);
