...
Процедура ПриЗаполненииОграниченияДоступа(Ограничение) Экспорт
	...
	|	И ЗначениеРазрешено(Запасы.СтруктурнаяЕдиница, ПустаяСсылка КАК Истина, Null КАК Истина)";
...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	Пока Выборка.Следующий() Цикл  
		Если Выборка.ЭтоВозвратВзаиморасчеты Тогда
			
			СтрокаРасчетов = ТаблицаРасчетыСПокупателями.Найти(Выборка.ТипРасчетов, "ТипРасчетов");
			...
			Если СтрокаРасчетов <> Неопределено Тогда
				...
				НоваяПроводка.Сумма = -СтрокаРасчетов.Сумма;
				Если НоваяПроводка.СчетДт.Валютный Тогда
					НоваяПроводка.СуммаВалДт = -СтрокаРасчетов.СуммаВал; 
				КонецЕсли;	
				Если НоваяПроводка.СчетКт.Валютный Тогда
					НоваяПроводка.СуммаВалКт = -СтрокаРасчетов.СуммаВал; 
				КонецЕсли;	
			КонецЕсли;
