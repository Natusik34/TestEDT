...
Процедура ЗаполнитьДобавленнуюКолонкуТаблицы(СтрокаТаблицы)
	...
	ГодовойРасчет = (КонецКвартала(СтрокаТаблицы.КонецПериода) = КонецГода(СтрокаТаблицы.КонецПериода));	
	...
	Иначе
		СтрокаТаблицы.ПредставлениеПериода =
		ПредставлениеПериодаОтчета(НачалоКвартала(СтрокаТаблицы.КонецПериода), СтрокаТаблицы.КонецПериода); 
	КонецЕсли;	
