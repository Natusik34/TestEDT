...
Процедура ПоказатьСодержаниеПисьма(ПринудительноОбновитьСодержание = Ложь)
	...
	Если Не ПринудительноОбновитьСодержание И ТекущаяСтрока = ЭтотОбъект.Предпросмотр_СсылкаНаСобытие Тогда
		Возврат;
	КонецЕсли;
	
	ЭтотОбъект.Предпросмотр_СсылкаНаСобытие = ТекущаяСтрока;
