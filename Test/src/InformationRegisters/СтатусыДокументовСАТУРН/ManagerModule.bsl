...
Функция РассчитатьСтатусыКПередаче(ДокументСсылка, НовыйСтатус, ОрганизацияСАТУРН = Неопределено) Экспорт
	...
	Если ОрганизацияСАТУРН = Неопределено Тогда
		РеквизитыОбъекта = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументСсылка, "ОрганизацияСАТУРН");
		ОрганизацияСАТУРН = РеквизитыОбъекта.ОрганизацияСАТУРН;
	КонецЕсли;
	...
	Иначе
		...
		ДальнейшиеДействия.Добавить(Перечисления.ДальнейшиеДействияПоВзаимодействиюСАТУРН.ОтменитеОперацию);
	КонецЕсли;
