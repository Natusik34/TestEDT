...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Запись.СпособПередачиТоваров <> Перечисления.СпособыПередачиТоваров.НеПередается
		И НЕ ТекущийОбъект.ОрганизацияВладелец.Пустая()
		И НЕ ТекущийОбъект.ОрганизацияПродавец.Пустая()
		...
		НовыйДоговор.Наименование = СтрШаблон(ШаблонНаименования, Запись.ОрганизацияВладелец, Запись.ОрганизацияПродавец);
		...
		НовыйДоговор.ВидЦен = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(Пользователи.АвторизованныйПользователь(), "ОсновнойВидЦенПродажи");
...
Процедура КомандаРазрешитьРедактированиеНастроек(Команда)
	...
	Если ТолькоПросмотр Тогда
		...
		Иначе
			...
			ТекстПредупреждения = НСтр("ru = 'Изменять действующие настройки может только пользователь с полными правами'");
