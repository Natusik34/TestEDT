...
#Область Проверки
	...
	// Если передали названия ИдентификаторМетаданных, ИдентификаторФормы или ИдентификаторыСобытийПриОткрытии более 50 символов,
	//  то отборы не сработают. В этом случае выдать исключение сразу.
	ПроверяемыеИдентификаторы = Новый Массив;
	ПроверяемыеИдентификаторы.Добавить(
		Новый Структура("Вид, Значение", "ИдентификаторФормы", ИдентификаторФормы));
	ПроверяемыеИдентификаторы.Добавить(
		Новый Структура("Вид, Значение", "ИдентификаторМетаданных", ИдентификаторМетаданных));
	Если ТипЗнч(ИдентификаторыСобытийПриОткрытии) = ТипСтрока Тогда
		ПроверяемыеИдентификаторы.Добавить(
			Новый Структура("Вид, Значение", "ИдентификаторСобытия", ИдентификаторыСобытийПриОткрытии));
	Иначе // Массив из Строка
		Для Каждого ТекущееСобытие Из ИдентификаторыСобытийПриОткрытии Цикл
			ПроверяемыеИдентификаторы.Добавить(
				Новый Структура("Вид, Значение", "ИдентификаторСобытия", ТекущееСобытие));
		КонецЦикла;
	КонецЕсли;
	Ошибки = Новый Массив;
	Для Каждого ТекущаяСтрока Из ПроверяемыеИдентификаторы Цикл
		Длина = СтрДлина(ТекущаяСтрока.Значение);
		Если Длина > 50 Тогда
			Ошибки.Добавить(ТекущаяСтрока.Вид + "=" + ТекущаяСтрока.Значение + " (" + Длина + ")");
		КонецЕсли;
	КонецЦикла;
	Если Ошибки.Количество() > 0 Тогда
		Ошибки.Вставить(0, НСтр("ru = 'Длина идентификаторов не должна превышать 50 символов:'", КодОсновногоЯзыка));
