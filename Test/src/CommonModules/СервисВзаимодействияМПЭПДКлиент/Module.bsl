...
Функция ПараметрыДействийДляМобильногоПриложения(Документ, НаименованиеМП = Неопределено, ИдентификаторЗаписиМП = Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.ЭлектронныйПутевойЛист") Тогда
		...
		Если РеквизитыДокумента.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЛ_Титул2")
	...
	Если ТитулСПользователямиМП <> Неопределено Тогда
		...
		ПользователиМП = СтруктураРеквизитовТитула[ИмяТаблицыПользователейМП];
