...
//
Процедура ЗаполнитьСписокВыбораСерийНоменклатуры(Номенклатура, Характеристика, МассивСерий, ИсключаяСерии, ВидОперации) Экспорт
...
Процедура НайтиБонусыЛокально(Форма) Экспорт
	...
	Иначе	
		...
		Товары = Форма.Объект.Товары.Выгрузить(,"Номенклатура, Характеристика, Сумма, Цена, Количество, СуммаСкидкиОплатыБонусом");
...
Процедура РассчитатьАвтоматическиеСкидки(ФормаРМК) Экспорт
	...
	Если СтруктураСкидок.Свойство("ТаблицаПодарков") Тогда
		ОписаниеРанееВыданныхПодарков = СкидкиНаценкиСервер.УчестьРанееВыданныеПодарки(ФормаРМК.Объект.Товары, СтруктураСкидок.ТаблицаПодарков, , Истина);
		...
		Если ТоварыУдалить.Количество() > 0 Тогда
			Для Каждого СтрокаУдалить Из ТоварыУдалить Цикл
				УдалитьСтрокуТовара(ФормаРМК, СтрокаУдалить, ФормаРМК.Объект.Товары);
			КонецЦикла;
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПараметрыРасчета.ДисконтнаяКарта) Тогда
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ФормаРМК, "МаксимальнаяСуммаОплатыБонусами") Тогда
...
Процедура ЗаполнитьКодыМаркировкиПоДаннымИБ(Форма, ТекущаяСтрока, КодыМаркировки) Экспорт
	...
	Если ЗначениеЗаполнено(ТекущаяСтрока.ТорговыйОбъект) Тогда
		ПараметрыСканирования.Склад = ТекущаяСтрока.ТорговыйОбъект;
	КонецЕсли;
...
Процедура ЗаполнитьВводДисконтныхКарт(ВводДисконтныхКарт) Экспорт
	...
	ВводДисконтныхКарт = Константы.ФункциональнаяОпцияИспользоватьДисконтныеКарты.Получить();
...
Процедура ПереопределитьНастройкиПомощникРМК(Форма) Экспорт
	...
	Форма.Элементы.СтраницаНастройкаРМКСкупка.Видимость = Истина;
	Форма.Элементы.СтраницаНастройкаРМКЗаказы.Видимость = Истина;
	...
	Для Каждого СтрокаОплатыРМК Из Источник.Оплата Цикл
		...
		Если СтрокаОплатыРМК.ВидОплаты = Перечисления.ВидыБезналичныхОплат.Бонусы Тогда
			...
			Иначе
				КурсБонусов = 1;
			КонецЕсли;
			НоваяСтрокаОплатыЧекККМ.СуммаБонусов = СтрокаОплатыРМК.Сумма / КурсБонусов;
