...
Процедура ПриСозданииНаСервереФормыСпискаДокументов(Форма, Настройки) Экспорт
	...
	МодульИнтеграцияСАТУРН = Неопределено;
	Если ОбщегоНазначения.ПодсистемаСуществует("ГосИС.САТУРН") Тогда
		
		МодульИнтеграцияСАТУРН = ОбщегоНазначения.ОбщийМодуль("ИнтеграцияСАТУРН");
	...
	Для Каждого ИмяСписка Из Новый Структура(ИменаВсехСписков) Цикл
		...
		Если МодульИнтеграцияСАТУРН <> Неопределено Тогда
			МодульИнтеграцияСАТУРН.ДополнитьПараметрыДинамическогоСпискаФормыСпискаДокументов(Список);
		КонецЕсли;
