...
Асинх Функция ПодключитьУстройствоАсинх(ПараметрыПодключения) Экспорт
	...
	КонецЕсли;
	Попытка
		РезультатВызова = Ждать ОбъектДрайвера.ПодключитьАсинх(ПараметрыПодключения.ИДУстройства);
...
Асинх Функция ТестУстройстваАсинх(ПараметрыПодключения) Экспорт
	...
	Если Результат Тогда
		...
		РезультатВызова = Ждать ОбъектДрайвера.ТестУстройстваАсинх(РезультатТеста, АктивированДемоРежим);    
...
Асинх Функция ВыполнитьДополнительноеДействиеАсинх(ПараметрыПодключения, ПараметрыВыполнения) Экспорт
	...
	Если Результат Тогда
		...
		Попытка
			...
			РезультатВызова = Ждать ОбъектДрайвера.ВыполнитьДополнительноеДействиеАсинх(ИмяДействия);
			Результат = РезультатВызова.Значение;
		Исключение 
			ОписаниеОшибки = НСтр("ru='Ошибка вызова метода <ВыполнитьДополнительноеДействие>.'");
			РезультатВыполнения = РезультатОперацииНаОборудовании(Ложь, ОписаниеОшибки);
			Возврат РезультатВыполнения; 
		КонецПопытки;
		...
		Иначе
			ОписаниеОшибки = "";
			РезультатВызова = Ждать ОбъектДрайвера.ПолучитьОшибкуАсинх(ОписаниеОшибки);
...
Асинх Функция ОбработатьСобытиеRFID(ПараметрыПодключения, Событие, Данные) Экспорт
	...
	Если ОбщегоНазначенияБПОКлиент.ИспользуетсяСчитывательRFID() Тогда
		...
		РезультатВыполнения = Ждать ПолучитьДанныеМетокRFID(ПараметрыПодключения.ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации);
...
Асинх Функция ПолучитьОшибкуДрайвера(ОбъектДрайвера) 
	...
	РезультатВызова = Ждать ОбъектДрайвера.ПолучитьОшибкуАсинх("");
	РезультатВыполнения = РезультатОперацииНаОборудовании(Ложь, РезультатВызова.Параметры[0]);
	Возврат РезультатВыполнения;
...
Асинх Функция ШиринаСтроки(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)  
	...
	Попытка
		РезультатВызова = Ждать ОбъектДрайвера.ПолучитьШиринуСтрокиАсинх(ПараметрыПодключения.ИДУстройства, 0);
		ПараметрыПодключения.ШиринаСтроки = РезультатВызова.Параметры[1];     
	Исключение   
		Возврат ОшибкаВызоваДрайвера("ПолучитьШиринуСтроки", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
	...
	ИначеЕсли ПараметрыПодключения.ТипыОборудования.ККТ Тогда
		...
		Если ПараметрыПодключения.РевизияИнтерфейса >= 3004  Тогда
			...
			Если НЕ ПустаяСтрока(ТекущийСтатус.ИдентификаторЗапроса) Тогда
				...
				Исключение
					РезультатВыполнения = ОшибкаВызоваДрайвера("ПодтвердитьКМ", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())); 
					Возврат РезультатВыполнения;
				КонецПопытки;
			КонецЕсли;       
		КонецЕсли;
		...
		Исключение                   
			РезультатВыполнения = ОшибкаВызоваДрайвера("СформироватьЧек", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())); 
			Возврат РезультатВыполнения;
		КонецПопытки;
...
Асинх Функция ПечатьЧекаКоррекции(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)   
	...
	Исключение            
		РезультатВыполнения = ОшибкаВызоваДрайвера("НапечататьЧекВнесенияВыемки", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Асинх Функция ЗакрытьСмену(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	ИначеЕсли ПараметрыПодключения.ТипыОборудования.ККТ Тогда        
		...
		Попытка  
			...
			КонецЕсли;
		Исключение    
			Возврат ОшибкаВызоваДрайвера("ЗакрытьСмену", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()))
		КонецПопытки;
...
Асинх Функция ОткрытьСмену(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	ИначеЕсли ПараметрыПодключения.ТипыОборудования.ККТ Тогда        
		...
		ПараметрыККТ = "";
		Попытка          
			РезультатВызова = Ждать ОбъектДрайвера.ПолучитьПараметрыККТАсинх(ПараметрыПодключения.ИДУстройства, ПараметрыККТ);
		Исключение
			РезультатВыполнения = ОшибкаВызоваДрайвера("ПолучитьПараметрыККТ", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));  
			Возврат РезультатВыполнения; 
		КонецПопытки;
		...
		Попытка  
			...
			КонецЕсли;
		Исключение    
			Возврат ОшибкаВызоваДрайвера("ОткрытьСмену", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()))
		КонецПопытки;
...
Асинх Функция ОткрытьДенежныйЯщик(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		РезультатВыполнения = ОшибкаВызоваДрайвера("ОткрытьДенежныйЯщик", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Асинх Функция ОтчетОТекущемСостоянииРасчетов(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Если ПараметрыПодключения.ТипыОборудования.ККТ Тогда
		...
		РезультатXML = "";
		Попытка
			РезультатВызова = Ждать ОбъектДрайвера.ОтчетОТекущемСостоянииРасчетовАсинх(ПараметрыПодключения.ИДУстройства, ДанныеОперации.ПараметрыXML, РезультатXML);
			Если РезультатВызова.Значение Тогда   
				РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);   
			...
			КонецЕсли;
		Исключение
			РезультатВыполнения = ОшибкаВызоваДрайвера("ОтчетОТекущемСостоянииРасчетов", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
...
Асинх Функция ОтчетБезГашения(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	ИначеЕсли ПараметрыПодключения.ТипыОборудования.ККТ Тогда
		...
		Попытка
			...
			КонецЕсли;
		Исключение
			РезультатВыполнения = ОшибкаВызоваДрайвера("НапечататьОтчетБезГашения", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
...
Асинх Функция ПараметрыККТ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации);
	...
	Если ПараметрыПодключения.ТипыОборудования.ККТ Тогда
		...
		Попытка
			РезультатВызова = Ждать ОбъектДрайвера.ПолучитьПараметрыККТАсинх(ПараметрыПодключения.ИДУстройства, "");
		Исключение
			Возврат ОшибкаВызоваДрайвера("ПолучитьПараметрыККТ", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;  
...
Асинх Функция ТекущееСостояние(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		...
		КонецЕсли;    
	Исключение    
		Возврат ОшибкаВызоваДрайвера("ПолучитьТекущееСостояние", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()))
	КонецПопытки;
...
Асинх Функция ОперацияФискальногоНакопителя(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		...
		КонецЕсли;                            
	Исключение
		РезультатВыполнения = ОшибкаВызоваДрайвера("ОперацияФН", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Асинх Функция НапечататьКопиюЧека(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Если ДанныеОперации.Свойство("Аппаратно") И ДанныеОперации.Аппаратно Тогда
		...
		Попытка
			...
			КонецЕсли;
		Исключение
			РезультатВыполнения = ОшибкаВызоваДрайвера("НапечататьКопиюЧека", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
...
Асинх Функция ОткрытьСессиюРегистрацииКМ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		РезультатВыполнения = ОшибкаВызоваДрайвера("ОткрытьСессиюРегистрацииКМ", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Асинх Функция ЗакрытьСессиюРегистрацииКМ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		РезультатВыполнения = ОшибкаВызоваДрайвера("ЗакрытьСессиюРегистрацииКМ", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Асинх Функция ЗапросКМ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		РезультатВыполнения = ОшибкаВызоваДрайвера("ЗапросКМ", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Асинх Функция ЗапросКМБезОжиданияОтвета(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Если РезультатВыполнения.Результат Тогда
		...
		Попытка
			РезультатВызова = Ждать ОбъектДрайвера.ПолучитьРезультатыЗапросаКМАсинх(ПараметрыПодключения.ИДУстройства, РезультатОперацииXML, СтатусЗапроса);
			Если РезультатВызова.Значение Тогда                             
			...
			КонецЕсли;
		Исключение
			РезультатВыполнения = ОшибкаВызоваДрайвера("ПолучитьРезультатыЗапросаКМ", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки; 
...
Асинх Функция ПараметрыТерминала(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Если ПараметрыПодключения.РевизияИнтерфейса >= 3004 Тогда
		Попытка
			РезультатВызова = Ждать ОбъектДрайвера.ПараметрыТерминалаАсинх(ПараметрыПодключения.ИДУстройства, "");  
			РезультатОперацииXML = РезультатВызова.Параметры[1];
			Если РезультатВызова.Значение Тогда
			...
			КонецЕсли         
		Исключение  
			РезультатВыполнения = ОшибкаВызоваДрайвера("ПараметрыТерминала", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
	Иначе     
		Попытка
			...
			РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);
		Исключение          
			РезультатВыполнения = ОшибкаВызоваДрайвера("ПечатьКвитанцийНаТерминале", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
	КонецЕсли;
		...
		Попытка                  
			Если РевизияИнтерфейса >= 4000 Тогда    
				...
				Если Не ПустаяСтрока(РеквизитыКартыQR) И НЕ ПараметрыПодключения.ConsumerPresentedQR Тогда
					...
					Возврат РезультатВыполнения;   
				КонецЕсли;
				РезультатВызова = Ждать ОбъектДрайвера.ПолучитьПараметрыКартыАсинх(ПараметрыПодключения.ИДУстройства, РеквизитыКартыQR, ОтПоследнейОперации, НомерКарты, ХешНомерКарты, СсылкаНаПлатежныйСчет, ТипКарты, СвояКарта);
			ИначеЕсли РевизияИнтерфейса >= 3007 Тогда
				РезультатВызова = Ждать ОбъектДрайвера.ПолучитьПараметрыКартыАсинх(ПараметрыПодключения.ИДУстройства, ОтПоследнейОперации, НомерКарты, ХешНомерКарты, СсылкаНаПлатежныйСчет, ТипКарты, СвояКарта);
			Иначе
				РезультатВызова = Ждать ОбъектДрайвера.ПолучитьПараметрыКартыАсинх(ПараметрыПодключения.ИДУстройства, ОтПоследнейОперации, НомерКарты, ХешНомерКарты, ТипКарты, СвояКарта);
			КонецЕсли;
			Если РезультатВызова.Значение Тогда     
				РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);  
				Если РевизияИнтерфейса >= 4000 Тогда   
			...
			КонецЕсли;
		Исключение      
			РезультатВыполнения = ОшибкаВызоваДрайвера("ПолучитьПараметрыКарты", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
	Иначе     
...
Асинх Функция ОперацииПоКартам(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Иначе               
		...
		КонецЕсли;
		Попытка
			РезультатВызова = Ждать ОбъектДрайвера.ПолучитьОперацииПоКартамАсинх(ПараметрыПодключения.ИДУстройства, "");  
			Если РезультатВызова.Значение Тогда        
				РезультатОперацииXML = РезультатВызова.Параметры[1];
				РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);   
			...
			КонецЕсли         
		Исключение  
			РезультатВыполнения = ОшибкаВызоваДрайвера("ПолучитьОперацииПоКартам", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
	КонецЕсли;
...
Асинх Функция ЭквайринговаяОперация(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации, Команда)
	...
	Исключение
		РезультатВыполнения = ОшибкаВызоваДрайвера(Команда, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Асинх Функция АварийнаяОтменаОперации(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		...
		КонецЕсли;
	Исключение  
		РезультатВыполнения = ОшибкаВызоваДрайвера("АварийнаяОтменаОперации", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Асинх Функция ИтогиДняПоКартам(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	ТекстСлипЧека = "";
	Попытка            
		РезультатВызова = Ждать ОбъектДрайвера.ИтогиДняПоКартамАсинх(ПараметрыПодключения.ИДУстройства, ТекстСлипЧека);
		Если РезультатВызова.Значение Тогда                    
			РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);  
		...
		КонецЕсли;
	Исключение  
		РезультатВыполнения = ОшибкаВызоваДрайвера("ИтогиДняПоКартам", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Асинх Функция ПараметрыВывода(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Если ПараметрыПодключения.РевизияИнтерфейса >= 3004 Тогда
		Попытка
			РезультатВызова = Ждать ОбъектДрайвера.ПолучитьПараметрыДисплеяПокупателяАсинх(ПараметрыПодключения.ИДУстройства, "");   
			РезультатОперацииXML = РезультатВызова.Параметры[1];
			Если РезультатВызова.Значение Тогда
				ПараметрыТерминала = ОбщегоНазначенияБПОКлиент.ПрочитатьКорневойЭлементXML(РезультатОперацииXML);
			...
			КонецЕсли         
		Исключение
			РезультатВыполнения = ОшибкаВызоваДрайвера("ПолучитьПараметрыДисплеяПокупателя", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
	Иначе
		Попытка
			РезультатВызова = Ждать ОбъектДрайвера.ПолучитьПараметрыВыводаАсинх(ПараметрыПодключения.ИДУстройства, 20, 2); 
			Если РезультатВызова.Значение Тогда
			...
			КонецЕсли;
		Исключение          
			РезультатВыполнения = ОшибкаВызоваДрайвера("ПолучитьПараметрыВывода", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
	КонецЕсли; 
...
Асинх Функция ВывестиСтрокуНаДисплейПокупателя(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		...
		КонецЕсли;
	Исключение  
		РезультатВыполнения = ОшибкаВызоваДрайвера("ВывестиСтрокуНаДисплейПокупателя", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Асинх Функция ВывестиQRКодНаДисплейПокупателя(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		...
		КонецЕсли;
	Исключение  
		РезультатВыполнения = ОшибкаВызоваДрайвера("ВывестиQRКодНаДисплейПокупателя", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Асинх Функция ВывестиHTMLНаДисплейПокупателя(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		...
		КонецЕсли;
	Исключение  
		РезультатВыполнения = ОшибкаВызоваДрайвера("ВывестиHTML", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Асинх Функция ВывестиСтатусНаДисплейПокупателя(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка                                                                                    
		...
		КонецЕсли;
	Исключение    
		РезультатВыполнения = ОшибкаВызоваДрайвера("ОтобразитьСтатусОперации", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Асинх Функция ВывестиСтатусРабочегоМестаНаДисплейПокупателя(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка                                                                                    
		...
		КонецЕсли;
	Исключение    
		РезультатВыполнения = ОшибкаВызоваДрайвера("ОтобразитьСтатусРабочегоМеста", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Асинх Функция ОчиститьДисплейПокупателя(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		...
		КонецЕсли;
	Исключение  
		РезультатВыполнения = ОшибкаВызоваДрайвера("ОчиститьДисплейПокупателя", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Асинх Функция ПараметрыВесов(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Если ПараметрыПодключения.РевизияИнтерфейса >= 4000 Тогда
		Попытка
			РезультатВызова = Ждать ОбъектДрайвера.ПараметрыВесовАсинх(ПараметрыПодключения.ИДУстройства, "");   
			РезультатОперацииXML = РезультатВызова.Параметры[1];
			Если РезультатВызова.Значение Тогда
			...
			КонецЕсли         
		Исключение
			РезультатВыполнения = ОшибкаВызоваДрайвера("ПараметрыВесов", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
	Иначе 
		РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);
...
Асинх Функция ПолучитьВес(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Вес = 0;   
	Попытка
		РезультатВызова = Ждать ОбъектДрайвера.ПолучитьВесАсинх(ПараметрыПодключения.ИДУстройства, Вес);
		Если РезультатВызова.Значение Тогда                    
		...
		КонецЕсли;
	Исключение  
		РезультатВыполнения = ОшибкаВызоваДрайвера("ПолучитьВес", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Асинх Функция Тарировать(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)   
	...
	Попытка
		...
		КонецЕсли;
	Исключение  
		РезультатВыполнения = ОшибкаВызоваДрайвера("УстановитьВесТары", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Асинх Функция ВыгрузитьТовары(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Для Каждого ДанныеДляВыгрузки Из ДанныеОперации.ПакетыXML  Цикл
		...
		Исключение                  
			РезультатВыполнения = ОшибкаВызоваДрайвера("ВыгрузитьТовары", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			Возврат РезультатВыполнения;
		КонецПопытки;
...
Асинх Функция ОчиститьТоварыВВесах(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		...
		КонецЕсли;
	Исключение  
		РезультатВыполнения = ОшибкаВызоваДрайвера("ОчиститьТовары", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Асинх Функция ВыгрузитьИзображенияТоваров(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Для Каждого ДанныеДляВыгрузки Из ДанныеОперации.ПакетыXML  Цикл
		...
		Исключение                  
			РезультатВыполнения = ОшибкаВызоваДрайвера("ВыгрузитьИзображенияТоваров", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			Возврат РезультатВыполнения;
		КонецПопытки;
...
Асинх Функция РаспознатьАсинх(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		РезультатВызова = Ждать ОбъектДрайвера.РаспознатьАсинх(ПараметрыПодключения.ИДУстройства, Обучение, Варианты, РаспознанныеТоварыXML);
		Если РезультатВызова.Значение Тогда                    
...
Асинх Функция ПодтвердитьРаспознаваниеАсинх(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)   
	...
	Попытка
		...
		КонецЕсли;
	Исключение  
		Результат = ОшибкаВызоваДрайвера("ПодтвердитьРаспознавание", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Асинх Функция ОчиститьТаблицу(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		...
		КонецЕсли;
	Исключение  
		РезультатВыполнения = ОшибкаВызоваДрайвера("ОчиститьТаблицу", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Асинх Функция ЗагрузитьТаблицу(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка                     
		ДанныеЗагрузки = ""; 
		РезультатВызова = Ждать ОбъектДрайвера.ЗагрузитьТаблицуАсинх(ПараметрыПодключения.ИДУстройства, ДанныеЗагрузки);
		Если РезультатВызова.Значение Тогда                    
			РезультатВыполнения = РезультатОперацииНаОборудовании(Истина); 
		...
		КонецЕсли;
	Исключение  
		РезультатВыполнения = ОшибкаВызоваДрайвера("ЗагрузитьТаблицу", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Асинх Функция ВыгрузитьТаблицу(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Для Каждого ДанныеДляВыгрузки Из ДанныеОперации.ПакетыXML  Цикл
		...
		Исключение
			РезультатВыполнения = ОшибкаВызоваДрайвера("ВыгрузитьТаблицу", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			Возврат РезультатВыполнения;
		КонецПопытки;
...
Асинх Функция ИнициализацияПринтера(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка       
		...
		КонецЕсли;
	Исключение
		РезультатВыполнения = ОшибкаВызоваДрайвера("ИнициализацияПринтера", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;       
...
Асинх Функция ПечатьЭтикеток(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Для Счетчик = 0 По КоличествоПакетов Цикл
		...
		Исключение
			РезультатВыполнения = ОшибкаВызоваДрайвера("ПечатьЭтикеток", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));   
			Возврат РезультатВыполнения;
		КонецПопытки;
...
//
Асинх Функция ПолучитьДанныеМетокRFID(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации) 
	
	ТаблицаМеток = "";
	ИДПакета = ДанныеОперации.ИДПакета;      
	
	Попытка       
		РезультатВызова = Ждать ОбъектДрайвера.ПолучитьДанныеМетокRFIDАсинх(ПараметрыПодключения.ИДУстройства, ИДПакета, ТаблицаМеток);
		Если РезультатВызова.Значение Тогда                    
			РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);   
		...
		КонецЕсли;
	Исключение
		РезультатВыполнения = ОшибкаВызоваДрайвера("ПолучитьДанныеМетокRFID", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;       
...
Асинх Функция ОткрытьСессиюRFID(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка       
		...
		КонецЕсли;
	Исключение
		РезультатВыполнения = ОшибкаВызоваДрайвера("ОткрытьСессиюRFID", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;       
...
Асинх Функция ЗакрытьСессиюRFID(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
 	...
 	Попытка       
		...
		КонецЕсли;
	Исключение
		РезультатВыполнения = ОшибкаВызоваДрайвера("ЗакрытьСессиюRFID", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;       
...
Асинх Функция ЗаписатьДанныеВМеткуRFID(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка       
		...
		КонецЕсли;
	Исключение
		РезультатВыполнения = ОшибкаВызоваДрайвера("ЗаписатьДанныеВМеткуRFID", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;       
