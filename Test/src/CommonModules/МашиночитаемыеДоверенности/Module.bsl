...
// Определяет признак передоверия 
//
// Параметры:
...
//  Булево
Функция ЭтоПередоверие(ДанныеXDTO) Экспорт
	
	Передоверие = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеXDTO, "Документ.Передов");
	
	Возврат Передоверие <> Неопределено;
...
Функция ВыгрузитьЗаявлениеНаОтменуМЧД(НомерДоверенности, ПричинаОтмены) Экспорт
	...
	Если Не ТипЗнч(ДанныеДоверенности) = Тип("Структура") Тогда
		...
		ИначеЕсли ДанныеДоверенности.Документ.Довер.СвДоверит[0].ТипДоверит = "3" Тогда
			...
			КонецЕсли;
			ЗаписьXML.ЗаписатьАтрибут("НаимИП", НаименованиеИП);
...
Функция РезультатПроверкиДоверенности(ДанныеДоверенности, РезультатПроверкиПодписи, КонтекстДиагностики) Экспорт
	...
	ЭтоПередоверие = ЭтоПередоверие(РезультатЧтения.ОбъектМЧД);
