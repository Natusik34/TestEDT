...
Процедура ЗаполнитьСопоставленнуюПродукциюВДокументе(ТабличнаяЧасть, НовыйДокумент) Экспорт
	...
	|		ПО ПолноеСоответствие.НомерСтроки = ТабличнаяЧасть.НомерСтроки
	|ГДЕ
	|	ПолноеСоответствие.НомерСтроки ЕСТЬ NULL
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ПолноеСоответствие.ПАТ КАК ПАТ,
	|	ПолноеСоответствие.НомерСтроки КАК НомерСтроки
	|ИЗ
	...
	Запрос.УстановитьПараметр("Товары", ТабличнаяЧасть.Выгрузить(,"НомерСтроки, Номенклатура, Характеристика, Серия"));
	...
	Пока ВыборкаСтроки.Следующий() Цикл
		...
		ВыборкаПАТ = ВыборкаСтроки.Выбрать();
		КоличествоСопоставлено = ВыборкаПАТ.Количество();
		...
		Пока ВыборкаПАТ.Следующий() Цикл
			Если КоличествоСопоставлено = 1 И НовыйДокумент Тогда
				СтрокаТЧ.ПАТ = ВыборкаПАТ.ПАТ;
			КонецЕсли;
			Если СтрокаТЧ.НоменклатураДляВыбора.НайтиПоЗначению(ВыборкаПАТ.ПАТ) = Неопределено Тогда
				СтрокаТЧ.НоменклатураДляВыбора.Добавить(ВыборкаПАТ.ПАТ);
			КонецЕсли;
		КонецЦикла;
...
Процедура ЗаполнитьСопоставленныеТовары(ТабличнаяЧасть, НоменклатураДляВыбора) Экспорт
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ТабличнаяЧасть[0], "ТипИзмеряемойВеличиныСАТУРН") Тогда
			...
			Для Каждого СтрокаТаблицы Из ТабличнаяЧасть Цикл
				...
				ДанныеУпаковки = ИнтеграцияИСКлиентСервер.ПолучитьДанныеУпаковки(
					Данные, СтрокаТаблицы.Номенклатура);
...
Процедура ДополнитьПараметрыДинамическогоСпискаФормыСпискаДокументов(Список) Экспорт
	
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
		Список,
		"ПустойТорговыйОбъект",
		ИнтеграцияИС.НезаполненныеЗначенияОпределяемогоТипа("ТорговыйОбъектСАТУРН"));
	...
	Для Каждого КлючИЗначение Из ВозвращаемоеЗначение Цикл
		...
		Если ЗначениеЗаполнено(ПараметрПолученияСопоставлений.МестоХранения) Тогда
			...
			Для Каждого СтрокаТаблицы Из ПоискСтрок Цикл
				...
				ЗаполнитьСсылкуИПредставлениеКлассификатора(
					ПараметрПолученияСопоставлений.МестоХранения,
					СтрокаТаблицы.ПроизводственныйОбъект,
