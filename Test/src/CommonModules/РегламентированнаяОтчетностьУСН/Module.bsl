...
Функция ВыполнитьРасчетАвансовогоПлатежа(Организация, ПериодРасчета, СобытиеКалендаря = Неопределено) Экспорт
	...
	СтруктураРасчетаАвансовогоПлатежа.НалогДляУведомления = Окр(Макс(0,СтруктураРасчетаАвансовогоПлатежа.ВсегоКУплатеАП - НалоговыйВычет) - СтруктураРасчетаАвансовогоПлатежа.АвансовыеПлатежиИсчисленные);	
...
Функция ВыполнитьРасчетВзносовВПФРиФСС(Организация, ПериодРасчета, СобытиеКалендаря = Неопределено) Экспорт
	...
	ИначеЕсли Год(ПериодРасчета) >= 2023 Тогда	
		...
		Если РегламентированнаяОтчетностьУСН.УпрощенныйЗачетФиксированныхВзносов(Организация, ПериодРасчета)
			...
			ВзносыПоЕдиномуТарифуПодлежащиеУплате = РассчитатьФиксированныеСтраховыеВзносы(ПериодРасчета, Организация).ЕдиныйТарифВсего;
...
Функция РассчитатьФиксированныеСтраховыеВзносыПосле2023(Знач Период, Организация = Неопределено)
	...
	Для НомерМесяца = ПервыйМесяц По 12 Цикл
		...
		Если ЗначениеЗаполнено(Организация) Тогда
			...
			Если ЗначениеЗаполнено(ДатаРегистрации)
			И НачалоМесяца < ДатаРегистрации
			И ДатаРегистрации <= НачалоДня(КонецМесяца) Тогда
...
Функция РазмерыСтраховыхВзносов(СтраховойПериод, Организация)
	...
	ИначеЕсли ГодСтраховогоПериода = 2024 Тогда
		...
		СтраховыеВзносы.ЕдиныйТариф = 49500;		
	Иначе
