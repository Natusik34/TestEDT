...
Функция ОшибкиНедоступныеДляПовторнойОтправкиДокумента() Экспорт
	...
	СписокОшибок.Вставить(4,   "Проверка отсутствия регистрации УД (Документ $docId не может быть повторно обработан");
	...
	СписокОшибок.Вставить(72,  "Проверка по типу документа (Некорректный тип документа: $docType)");
	СписокОшибок.Вставить(76,  "Проверка ID документа (Документ с id $id не найден)");
...
Процедура ДобавитьВРегистрДляОтслеживанияСоответствияТребованиямГИСМТ(Знач МассивДокументов)
	...
	Для Каждого Документ Из МассивДокументов Цикл
		...
		Если СуществующийРезультат = Неопределено
			И Не (СтатусЭДО = "НеНачат" Или СтатусЭДО = "Утвержден" Или СтатусЭДО = "Получен") Тогда
			Возврат;
		КонецЕсли;
