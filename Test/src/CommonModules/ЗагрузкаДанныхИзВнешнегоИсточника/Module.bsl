...
Функция ПодготовитьТабличныйДокументОтчетOzon(Знач ТабличныйДокумент, ЗагружатьПродажи = Истина,
	ЗагружатьВозвраты = Ложь)
	
	Перем ОбластьУдаления;
	Перем НайденВсего;    
		
	ТаблицаДанных = ПрочитатьТабличныйДокументВТаблицу(ТабличныйДокумент);
	...
	Для Каждого СтрокаДанных Из ТаблицаДанных Цикл
		...
		Иначе
			...
			ИначеЕсли НайденаСтрокаЗаголовкаТаблицы И НайденаВтораяСтрокаЗаголовкаТаблицы И НайденаСтрокаНумерацииКолонокТаблицы Тогда	
				...
				РазобратьСтрокуТабличнойЧасти(ТаблицаДанных, СтрокаДанных, ТаблицаЗаголовков, НомерСтрокиТовара, ТаблицаТоваров, НомерКолонкиДоплатаВозврат);
...
Процедура ДобавитьСтрокиВТаблицуЗаголовковОтчетOzon(ТаблицаЗаголовков)
	...
	// количество
	НоваяСтрокаЗаголовков = ТаблицаЗаголовков.Добавить();
	НоваяСтрокаЗаголовков.НазваниеКолонки 	= "КоличествоВозврат";  // АПК:1036 проверка орфографии не требуется.
	НоваяСтрокаЗаголовков.СтрокаПоиска 		= "колво";				// АПК:1036 проверка орфографии не требуется.	
...
Процедура РазобратьСтрокуТабличнойЧасти(ТаблицаДанных, СтрокаДанных, ТаблицаЗаголовков, НомерСтрокиТовара, 
	ТаблицаТоваров, НомерКолонкиДоплатаВозврат)
	...
	Для Каждого ЭлементЗаголовка Из МассивЗаголовков Цикл
		...
		Иначе
			Если ЭлементЗаголовка.ТипДанных = "Число" Тогда
				...
				ИначеЕсли ЭлементЗаголовка.НазваниеКолонки = "Доплата" И ЗначениеЯчейки = "" Тогда
					// В строке нет продаж, но есть возврат
					ЗначениеДоплаты = 0; 	
					Если НомерКолонкиДоплатаВозврат <> 0  Тогда
						...
						КонецЕсли;
					КонецЕсли;  				
					ЗначениеЯчейки = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(ЗначениеДоплаты) // СуммаДоплатыВозврат 
				Иначе
...
Процедура ЗаполнитьРасшифровкиВТабличномДокументе(ТабличныйДокумент, КоличествоКолонок, НастройкиЗагрузкиДанных) Экспорт
	...
	Для НомерКолонки = 1 По КоличествоКолонок Цикл
		...
		Если ЗагрузкаИзФайлаOzon Тогда			
			Если ОбластьПриемник.Текст = "НаимТов" Тогда  								// АПК:1297 не локализуется
				ИсходнаяОбласть.Примечание.Текст = НСтр("ru ='Наименование'");
			ИначеЕсли ОбластьПриемник.Текст = "Возвращено" Тогда  						// АПК:1297 не локализуется
