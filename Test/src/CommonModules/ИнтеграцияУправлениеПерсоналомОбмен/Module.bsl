...
Функция РезультатВыполненияОбмена(Приложение, ВестиПротокол)
	...
	Если Не БылиОшибки Тогда
		...
		// Выгрузка изменений.
		Попытка
			БылиОшибкиЭтапа = ВыгрузитьИзменения(МенеджерОбмена);
			БылиОшибки = БылиОшибки Или БылиОшибкиЭтапа;
		Исключение
			БылиОшибки = Истина;
			ЗаписьЖурналаРегистрации(ИменаСобытийЖР.ПрочиеСобытия,
...
Функция ВыгрузитьИзменения(МенеджерОбмена)
	
	БылиОшибки = ВыгрузитьОбъектыПоПравилам(МенеджерОбмена);
...
Функция ВыгрузитьОбъектыПоПравилам(МенеджерОбмена)
	
	БылиОшибки = Ложь;
	
	ДанныеДляВыгрузки = ДанныеДляВыгрузкиПоПравилам(МенеджерОбмена);
	...
	Если ЗначениеЗаполнено(ДанныеОбъектов) Тогда
		...
		ТипыОбъектов = ИнтеграцияУправлениеПерсоналом.ТипыОбъектовВыгружаемыхПоПравилам(Приложение);
	...
	Попытка
		// ДанныеДляВыгрузки
		// таблицы ОтменитьРегистрацию и ТаблицаИзменений
...
Функция ОбработатьЗагруженныеИзменения(МенеджерОбмена, ОбъектВсеИзменения, ПовторнаяЗагрузка, ПовторнаяОбработка) Экспорт
...
// Получение данных выгрузки объектов по правилам обмена.
//
// Параметры:
// 		МенеджерОбмена
//
// Возвращаемое значение:
// 		Структура - свойства
// 			* ДанныеОбъектовПоТипам - Соответствие - данные в разрезе типов объектов
...
//
Функция ДанныеДляВыгрузкиПоПравилам(МенеджерОбмена)
	...
	// Инициализации таблицы для отмены регистрации изменений, если объекта не соответсвует правиалам.
	...
	ДанныеОбъектовПоТипам = Новый Соответствие;
	Для каждого ТипОбъекта Из ТипыОбъектов Цикл
		
		ДанныеПоТипуОбъекта = Новый Структура("ТаблицаДанных,ОбъектыКУдалению");
		...
		// Структура НовыеДанныеДляВыгрузки()
		ДанныеОбъектов = ДанныеОбъектовДляВыгрузки(МенеджерОбмена, Изменения, ТипОбъекта);
		Если ЗначениеЗаполнено(ДанныеОбъектов) Тогда
			ДанныеПоТипуОбъекта.ТаблицаДанных = ДанныеОбъектов.ТаблицаДанных;
			ДанныеПоТипуОбъекта.ОбъектыКУдалению = ОбъектыКУдалению;
			ДанныеОбъектовПоТипам.Вставить(ТипОбъекта, ДанныеПоТипуОбъекта);
			ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ДанныеОбъектов.ОтменитьРегистрацию, ОтменитьРегистрацию);
		КонецЕсли;
	
	КонецЦикла;
	
	ДанныеДляВыгрузки.ДанныеОбъектов 		= ДанныеОбъектовПоТипам;
	ДанныеДляВыгрузки.ОтменитьРегистрацию 	= ОтменитьРегистрацию;
	ДанныеДляВыгрузки.ТаблицаИзменений 		= ТаблицаИзменений;
...
Функция ДанныеСотрудников(МенеджерОбмена, ТаблицаИзменений)
	...
	Для каждого СтрокаТЗ Из ДанныеСотрудников Цикл
		...
		Если СотрудникиНеВыгружать[СтрокаТЗ.Сотрудник] = Истина Тогда
			...
			НоваяСтрока.ВерсияДанных 	= ТаблицаИзменений.Найти(СтрокаТЗ.Сотрудник, "Ссылка").ВерсияДанных;
			Продолжить;
		КонецЕсли;
...
Функция ДанныеГрафиковРаботы(ТаблицаИзменений, Приложение)
	...
	Данные = ИнтеграцияУправлениеПерсоналом.ДанныеГрафиковРаботы(ГрафикиРаботыСотрудников, Приложение);
