...
Функция ВложенныеШтрихкодыУпаковок(Упаковки, ПараметрыСканирования = Неопределено, ЗаполнитьСведенияПоШтрихкодамEAN = Ложь, СортироватьДанные = Ложь) Экспорт
	...
	Если ТипЗнч(Упаковки[0]) <> Тип("СправочникСсылка.ШтрихкодыУпаковокТоваров") Тогда
		...
		Для Каждого ЭлементМассиваУпаковок Из Упаковки Цикл
			...
			Если ЭлементДанныхГИСМТ.ДанныеЗапросаГИСМТ <> Неопределено Тогда
				
				ДанныеЗапросаГИСМТ = ЭлементДанныхГИСМТ.ДанныеЗапросаГИСМТ.Получить();
				
				Если ДанныеЗапросаГИСМТ <> Неопределено Тогда
					ЗаполнитьЗначенияСвойств(ЭлементДанныхГИСМТ, ДанныеЗапросаГИСМТ);
				КонецЕсли;
				
			КонецЕсли;
			
			Если ЗначениеЗаполнено(ЭлементДанныхГИСМТ.ИдентификаторЗапросаГИСМТ) Тогда
...
Функция ИнициализацияТаблицыДанныхКодовМаркировки(ИндексироватьТаблицы = Истина) Экспорт
	...
	ДанныеКодовМаркировки.Колонки.Добавить("ИдентификаторЗапросаГИСМТ",        Новый ОписаниеТипов("Строка"));
	ДанныеКодовМаркировки.Колонки.Добавить("ДатаВремяПолученияИдентификатора", Новый ОписаниеТипов("Дата",,, Новый КвалификаторыДаты(ЧастиДаты.ДатаВремя)));
...
// * ИдентификаторЗапросаГИСМТ - ОпределяемыйТип.УникальныйИдентификаторИС - идентификатор ГИС МТ
// * ДатаВремяПолученияИдентификатора - Дата - дата получения идентификатора ГИС МТ
Функция НовыйЭлементДополненияВложенныхШтрихкодовУпаковокДанныеГИСМТ() Экспорт
	
	ВозвращаемоеЗначение = Новый Структура();
	ВозвращаемоеЗначение.Вставить("ДатаВремяПолученияИдентификатора");
	ВозвращаемоеЗначение.Вставить("ИдентификаторЗапросаГИСМТ");
	ВозвращаемоеЗначение.Вставить("ДанныеЗапросаГИСМТ");
...
Функция ОбщиеКолонкиКоллекцийВложенныхШтрихкодов()
	...
	КоллекцияОбязательныхКолонок.Вставить("ИдентификаторЗапросаГИСМТ");
	КоллекцияОбязательныхКолонок.Вставить("ДатаВремяПолученияИдентификатора");
...
Процедура ОбработатьШтрихкодыПоКегамНаОборудованииРозливаИСМП(ДанныеПоШтрихкодам, ДанныеШтрихкодаСписок, ПараметрыСканирования, КешДанныхРазбора = Неопределено, ИсходныеДанныеКодовМаркировки = Неопределено)
	...
	ИначеЕсли КодыМаркировки.Количество() = 1 Тогда
		...
		ДанныеШтрихкодаСписок[0].ШтрихкодBase64 = ШтрихкодированиеИСКлиентСервер.ШтрихкодВBase64(ДанныеШтрихкодаСписок[0].Штрихкод);
