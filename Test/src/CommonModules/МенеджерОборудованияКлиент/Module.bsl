...
Функция ПолучитьТипыОборудования(ТипыПО) Экспорт
	...
	Если ТипЗнч(ТипыПО) = Тип("Строка") Тогда    
		...
		Для Каждого ТипОборудования Из ТипыВМассиве Цикл
			МассивТипов.Добавить(Соответствие.Получить(ТипОборудования));
		КонецЦикла;
...
Асинх Функция УстановитьДрайверОборудованияАсинх(ИдентификаторДрайвера) Экспорт
	...
	Если ДанныеДрайвера.ПодключениеИзМакета Тогда
		...
		Иначе
			...
			Результат = Ждать ВнешниеКомпонентыБПОКлиент.УстановитьКомпонентуИзМакетаАсинх(ДанныеДрайвера.ИмяМакетаДрайвера, ПараметрыУстановки);
		КонецЕсли;
	Иначе
		ПараметрыУстановки = ВнешниеКомпонентыБПОКлиент.ПараметрыУстановки();
		...
		ПараметрыУстановки.ТекстПояснения = ТекстСообщения; 
		Результат = Ждать ВнешниеКомпонентыБПОКлиент.УстановитьКомпонентуАсинх(ДанныеДрайвера.ИдентификаторОбъекта, ДанныеДрайвера.ВерсияДрайвера, ПараметрыУстановки);
	КонецЕсли;
...
Процедура НачатьОтключениеОборудованияЗавершение(РезультатВыполнения, Контекст) Экспорт
	...
	ОтключаемоеУстройство = Контекст.ОтключаемоеУстройство;
...
Процедура НачатьВыполнениеКоманды_ПечатьДополнительныхДокументов(РезультатВыполнения, ПараметрыВыполнение) Экспорт
	...
	Иначе
		// Отключение оборудования при завершение
		ОтключитьОборудованиеПослеВыполнениеКоманды(ПараметрыВыполнение);
		// Завершение с ошибкой
		ТекстСообщения = СтрШаблон(НСтр("ru='При подключении печатающего устройства произошла ошибка: %1'"), РезультатВыполнения.ОписаниеОшибки); 
		РезультатОперации = МенеджерОборудованияКлиентСервер.ПараметрыВыполненияОперацииНаОборудовании(Ложь, ТекстСообщения);
		РезультатОперации.ИдентификаторУстройства = РезультатВыполнения.ИдентификаторУстройства;
		ВыполнитьОбработкуОповещения(ПараметрыВыполнение.ПараметрыВыполнениеКоманды.ОповещениеПриЗавершении, РезультатОперации);
	КонецЕсли;
...
Процедура НачатьВыполнениеКоманды_ПечатьДополнительныхДокументовЗавершение(РезультатВыполнения, ПараметрыВыполнение) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		...
		Иначе
			// Отключение оборудования.
			ОтключитьОборудованиеПослеВыполнениеКоманды(ПараметрыВыполнение.ПараметрыВыполнениеКоманды);
			// Завершение команды.
			ЗавершениеВыполнениеКоманды(ПараметрыВыполнение.РезультатВыполнения, ПараметрыВыполнение.ПараметрыВыполнениеКоманды);
		КонецЕсли;
	Иначе
		ТекстСообщения = СтрШаблон(
		...
		ПараметрыВыполнение.ПараметрыВыполнениеКоманды.ИдентификаторУстройства,
		ПараметрыВыполнение.ПараметрыВыполнениеКоманды.ПечатающееУстройство);
		ОбщегоНазначенияБПОКлиент.СообщитьПользователю(ТекстСообщения);
