...
Функция СокращеннаяИнформацияПоКМПриРозничнойПродаже(ДанныеПроверкиКМ, Организация = Неопределено, ТаймаутНаПродажу = Истина) Экспорт
	...
	Для Каждого СтрокаДанныхПроверкиКМ Из ДанныеПроверкиКМ Цикл
		...
		МассивКМ.Добавить(ШтрихкодированиеИСКлиентСервер.Base64ВШтрихкод(ПолныйКодКМ));
		СоответствиеПолногоКодаКМ.Вставить(СтрокаДанныхПроверкиКМ.НормализованныйШтрихкод, СтрокаДанныхПроверкиКМ);
						...
						Для Каждого ЭлементДанных Из ДанныеОбработки.codes Цикл
							...
							Иначе
								...
								Если ДанныеОбработки.Свойство("reqId") Тогда
									ПараметрыКодаМаркировки.ИдентификаторЗапросаГИСМТ = ДанныеОбработки.reqId;
								КонецЕсли;
								
								Если ДанныеОбработки.Свойство("reqTimestamp") Тогда
									ПараметрыКодаМаркировки.ДатаВремяПолученияИдентификатора = ИнтеграцияИС.ДатаИзСтрокиUNIX(ДанныеОбработки.reqTimestamp);
								КонецЕсли;
