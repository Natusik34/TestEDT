...
Процедура ПриПолученииСпискаШаблонов(Шаблоны) Экспорт
	...
	Шаблоны.Добавить(Метаданные.РегламентныеЗадания.ПолучениеСобытийМПЭПД.Имя);
КонецПроцедуры

Процедура ПриОпределенииПсевдонимовОбработчиков(СоответствиеИменПсевдонимам) Экспорт
	СоответствиеИменПсевдонимам.Вставить(Метаданные.РегламентныеЗадания.ОтправкаQRКодовЭПД.ИмяМетода);
	СоответствиеИменПсевдонимам.Вставить(Метаданные.РегламентныеЗадания.ПолучениеСобытийМПЭПД.ИмяМетода);
КонецПроцедуры
...
Процедура ЗаполнитьДополнительныхДанныеКарточкиЭПД(СообщениеЭДО, ДополнительныеДанные, ТипДокументаСтрокой) Экспорт
	...
	Если Выборка.Следующий() Тогда
		...
		Если ОбменСГИСЭПДКлиентСервер.ИспользуютсяДополнительныеРеквизитыЭПД() = Истина Тогда
...
Процедура ЗаполнитьВерсиюТитулаМобильногоПриложения(Форма, Знач ТекущийТитул) Экспорт
	...
	ИнформацияПоТитулуМП = СервисВзаимодействияМПЭПДКлиентСервер.ИнформацияПоТитулуМП(Форма.Объект.Ссылка, Форма.Объект.РольУчастника);
	Если ИнформацияПоТитулуМП = Неопределено Тогда
	...
	СоответствиеТитуловТипамМП = СоответствиеТитуловТипамМП();
	...
	Выборка = Запрос.Выполнить().Выбрать();
	ПривязкаПользователяНайдена = Ложь;
	МассивПолученныхТипов = Новый Массив;
	Пока Выборка.Следующий() Цикл
		Если ПривязкаПользователяНайдена = Ложь
		И (ЗначениеЗаполнено(Выборка.МобильноеПриложение)
			Или ИнформацияПоТитулуМП.СобытияПривязки.Найти(Выборка.ВидСообщения) <> Неопределено) Тогда
			Форма.МПНаименование = Выборка.МобильноеПриложение;
			Если ЗначениеЗаполнено(Выборка.Телефоны) Тогда
				Форма.МПТелефон = ОбменСГИСЭПДКлиентСервер.ПолучитьПредставлениеТелефона(Выборка.Телефоны);
				ФотоЗначениеХранилища = Выборка.Фото.Получить();
				Если ФотоЗначениеХранилища <> Неопределено Тогда
					Форма.МПФото = ПоместитьВоВременноеХранилище(ФотоЗначениеХранилища, Форма.УникальныйИдентификатор);
				Иначе
					Форма.МПФото = ПоместитьВоВременноеХранилище(СервисВзаимодействияМПЭПДКлиентСервер.КартинкаПустаяФотография(), Форма.УникальныйИдентификатор);
				КонецЕсли;
			КонецЕсли;
			Форма.МПИдентификатор = Выборка.ИдентификаторМП;
			Форма.МПИдентификаторЗаписи = Выборка.Идентификатор;
			ПривязкаПользователяНайдена = Истина;
		КонецЕсли;
		
		// В документе показываем только сообщения (титулы)
		Если Выборка.ВидСообщения <> Перечисления.ТипыВзаимодействияМПЭПД.Сообщение Тогда
			Продолжить;
		КонецЕсли;
		...
		Если СоздатьВерсиюПользователяМП Тогда	
			...
			Если ГруппаМобильноеПриложениеСтатус = Неопределено Тогда
				ГруппаМобильноеПриложениеСтатус = Форма.Элементы.Вставить(ИмяГруппыСтатусМП, 
													...
													Форма.Элементы.ГруппаНавигацийИМобильногоПриложения);
			...
			Если Принято = Ложь Тогда
				...
				Иначе
					ТекстКомментарияУОУ = СервисВзаимодействияМПЭПДКлиентСервер.ПредставлениеПустогоЗначенияДанныхМП();
				КонецЕсли;
				...
				Если СтрНачинаетсяС(КиЗ.Ключ, ИнформацияПоПрефиксамТитула.ВПрограмме) Тогда
					...
					Если ЭлементВвода <> Неопределено Тогда	
						...
						Если ГруппаНадписьДанныеМП = Неопределено Тогда
							...
							Если ТипЗначенияНаФорме = Тип("Строка")
								И ЭлементВвода.МногострочныйРежим = Истина Тогда
							...
							НадписьДанныеМП.Заголовок = ИнформацияПоТитулуМП.ЗаголовокПолей + РазделениеЗаголовкаиДанных 
																				+ ?(ЗначениеЗаполнено(ЗначениеРеквизита), 
																						ЗначениеРеквизита,
																						СервисВзаимодействияМПЭПДКлиентСервер.ПредставлениеПустогоЗначенияДанныхМП());		
...
Функция СоответствиеТитуловТипамМП()
	
	Результат = Новый Соответствие;
	
	Результат.Вставить("acceptance", Перечисления.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул2);
	Результат.Вставить("unloading", Перечисления.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул4);
...
Функция ОписаниеТипаДопРеквизита(Знач ТипСтрокой = Неопределено, Знач Квалификаторы = Неопределено) Экспорт
	...
	Иначе
		...
		Если ТипЗнч(Квалификаторы) = Тип("Массив") Тогда
			...
			КонецЕсли;
		КонецЕсли;
		Если ТипСтрокой = "Строка" Или ТипСтрокой = "Перечисление" Тогда
...
Функция ГруппыДанныхДополнительныхПолей(Знач ИмяДокумента, МакетСоответствиеИмен = Неопределено) Экспорт
	...
	УмяУзлаДополнительныхПолей = "ИнфПол";
	
	ОбластьНайдено = МакетСоответствиеИмен.НайтиТекст("/" + УмяУзлаДополнительныхПолей, , ОбластьКолонкаУзлов, Ложь, Ложь, Истина, Ложь);
	Пока ОбластьНайдено <> Неопределено Цикл
		Если СтрЗаканчиваетсяНа(ОбластьНайдено.Текст, "/" + УмяУзлаДополнительныхПолей) Тогда
		...
		КонецЕсли;												
		ОбластьНайдено = МакетСоответствиеИмен.НайтиТекст("/" + УмяУзлаДополнительныхПолей, ОбластьНайдено, ОбластьКолонкаУзлов, Ложь, Ложь, Истина, Ложь);
	КонецЦикла;
		...
		Для СчетчикСтрок = 0 По ОбластьТитула.Низ - ОбластьТитула.Верх Цикл
			...
			Для Каждого ЧастьПути Из МассивЧастейПути Цикл
				...
				Если СтрокаНайдено = Неопределено Тогда
					...
					Представление = ИмяГруппы;
					Если СтрНачинаетсяС(Представление, ИмяТитула) Тогда
						Представление = Прав(Представление, СтрДлина(Представление) - СтрДлина(ИмяТитула));
					КонецЕсли;
					
					ПредставлениеСПробелами = "";
					Для НомерСимвола = 1 По СтрДлина(Представление) - 1 Цикл
						ТекущийСимвол = Сред(Представление, НомерСимвола, 1);
						Если Прав(ПредставлениеСПробелами, 1) = " " Тогда
							ТекущийСимвол = НРег(ТекущийСимвол);
						КонецЕсли;
						ПредставлениеСПробелами = ПредставлениеСПробелами + ТекущийСимвол;
						СледующийСимвол = Сред(Представление, НомерСимвола + 1, 1);
						Если ВРег(СледующийСимвол) = СледующийСимвол Тогда
							ПредставлениеСПробелами = ПредставлениеСПробелами + " ";	
						КонецЕсли;	
					КонецЦикла;
					ПредставлениеСПробелами = ПредставлениеСПробелами + Прав(Представление, 1);
								
					СтрокаТекущая.Представление = ПредставлениеСПробелами;
