...
Процедура УстановитьЗначениеРеквизитаВСтруктуреФормы(Форма, ИмяЭлемента, Значение = Неопределено, Перезаписывать = Истина, КонтейнерОтображаемойВерсии = Неопределено) Экспорт
	...
	Если Перезаписывать = Истина Или ЗначениеЗаполнено(ТекущееЗначение) = Ложь Тогда
		...
		Если СтруктураРеквизитовФормы.Свойство(ИмяЭлемента) Тогда
			...
			Если СервисВзаимодействияМПЭПДКлиентСервер.ТитулПоддерживаетсяМобильнымПриложением(КонтейнерОтображаемойВерсии.ПрефиксТитула) Тогда
				...
				Если ЭлементДанныеМП <> Неопределено Тогда
					...
					Если ЗначениеМП = СервисВзаимодействияМПЭПДКлиентСервер.ПредставлениеПустогоЗначенияДанныхМП() Тогда
						ЗначениеМП = "";
					КонецЕсли;
...
Функция ОписаниеГруппыДанных(Знач ГруппаДанных, Знач ИмяОбъекта) Экспорт
	...
	СписокРеквизитов = "";
	СписокТаблиц = "";
	Если ГруппаДанных = "ТитулГрузоотправителяТранспортноеСредство"
		Или ГруппаДанных = "ТитулПеревозчикаТранспортноеСредство"
		Или ГруппаДанных = "ТитулПеревозчикаЗаменыТранспортноеСредство"
		Или ГруппаДанных = "ТранспортноеСредство" Тогда
		...
		СписокРеквизитов = "VIN,
							...
							|Тип,
							|ТипВладения";
		Если ГруппаДанных = "ТитулПеревозчикаЗаменыТранспортноеСредство" Тогда
			СписокТаблиц = "ТитулПеревозчикаЗаменыТранспортныеСредства";
		КонецЕсли;
	ИначеЕсли ГруппаДанных = "ТитулГрузоотправителяПараметрыТранспортногоСредства"
		Или ГруппаДанных = "ПараметрыТранспортногоСредства" Тогда
		...
		СписокРеквизитов = "Вместимость,
							...
							|ТипПогрузки,
							|ИныеПараметры";
	ИначеЕсли ГруппаДанных = "ТитулФрахтователяПараметрыТранспортногоСредства" Тогда
		Результат.Заголовок = "Параметры транспортных средств";	
		Результат.Тип = "ПараметрыТранспортногоСредства";
		Результат.ИмяФормы = ИмяОбъектаДляФормы + ".Форма." + Результат.Тип;
		Результат.ЕстьХранимыеДанные = Истина;
	...
	ИначеЕсли ГруппаДанных = "Водитель" Тогда
		...
		ИначеЕсли ИмяОбъектаДляФормы = "Документ.ЭлектронныйДоговорФрахтования"
			Или ИмяОбъектаДляФормы = "Документ.ЭлектронныйЗаказНаряд" Тогда 
			СписокТаблиц = "ТитулФрахтовщикаВодитель";
	...
	ИначеЕсли ГруппаДанных = "ТитулФрахтовщикаТранспортноеСредство" Тогда
		...
		Результат.ИмяФормы = ИмяОбъектаДляФормы + ".Форма." + Результат.Тип;
		Результат.ЕстьХранимыеДанные = Истина;
		СписокРеквизитов = "РегистрационныйНомер,
							|Вместимость,
							|Грузоподъемность,
							|Марка,
							|Тип";
