	...
	Для Каждого СтрокаДанныхПоШтрихкодам Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		Для Каждого СтрокаПодчиненнойУпаковки Из СтрокаДанныхПоШтрихкодам.СтрокаДерева.Строки Цикл
			...
			Если ПолучатьИдентификатор Тогда
				...
				СтруктураЗаполнения.Вставить("ИдентификаторЗапросаГИСМТ",        Неопределено);
				СтруктураЗаполнения.Вставить("ДатаВремяПолученияИдентификатора", Неопределено);
...
Функция ШтрихкодыУпаковокИзДокумента(Знач ДокументСсылка, Знач ПараметрыСканирования) Экспорт
	...
	ЕстьКолонкаДанныеЗапросаГИСМТ = МетаданныеДокумента.ТабличныеЧасти[ИмяТабличнойЧасти].Реквизиты.Найти("ДанныеЗапросаГИСМТ") <> Неопределено;
	...
	Если ИнтеграцияИСМПКлиентСерверПовтИсп.ПроверкаТоварныхГруппПриРозничнойПродажеГИСМТ()
		И ЕстьКолонкаДанныеЗапросаГИСМТ
		И ПараметрыСканирования.ВидОперацииИСМП = Перечисления.ВидыОперацийИСМП.ВыводИзОборотаРозничнаяПродажа Тогда
		ВыбираемыеПоля.Добавить(
			"Штрихкоды.ДанныеЗапросаГИСМТ КАК ДанныеЗапросаГИСМТ");
	КонецЕсли;
...
Процедура ДополнитьДанныеПоШтрихкодамДаннымиПулаКодовМаркировки(ДанныеПоШтрихкодам, ПараметрыСканирования, ВыполнитьПринудительно = Ложь) Экспорт
	...
	Если КодыМаркировки.Количество() > 0 Тогда
		...
		Для Каждого СтрокаТаблицы Из КодыМаркировкиВПуле Цикл
			...
			Если ПараметрыСканирования.ЭтоПечатьКодаМаркировкиИзПулаИСМП Или ЗначениеЗаполнено(СтрокаДанных.ИдентификаторЗапросаГИСМТ) Тогда
...
Процедура ЗаполнитьПараметрыДополненияВложенныхШтрихкодов(ПараметрыДополнения, ШтрихкодыУпаковок, ПараметрыСканирования) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ШтрихкодыУпаковок Цикл
		...
		Если Не ЭлементДополнения.ДанныеЗапросаГИСМТ = Неопределено Тогда
			
			ДанныеЗапросаГИСМТ = ЭлементДополнения.ДанныеЗапросаГИСМТ.Получить();
			
			Если Не ДанныеЗапросаГИСМТ = Неопределено Тогда
				
				ЗаполнитьЗначенияСвойств(ЭлементДополнения, ДанныеЗапросаГИСМТ);
