...
Функция ЗаполнитьСтрокуШаблона(СтрокаШаблона, ИсточникДанных)
	...
	Пока НЕ ПустаяСтрока(ИмяПоля) Цикл
		...
		КонецЕсли;
		Результат = СтрЗаменить(Результат, "{" + ИмяПоля + "}", Значение);
...
Функция НайтиПолеПодстановки(СтрокаШаблона)
	...
	ГдеНачалоПоля = СтрНайти(СтрокаШаблона, "{", НаправлениеПоиска.СНачала, 1);
	Если ГдеНачалоПоля > 0 Тогда
		ГдеКонецПоля = СтрНайти(СтрокаШаблона, "}", НаправлениеПоиска.СНачала, ГдеНачалоПоля);
		Результат = Сред(СтрокаШаблона, ГдеНачалоПоля + 1, ГдеКонецПоля - ГдеНачалоПоля - 1);
	КонецЕсли;
