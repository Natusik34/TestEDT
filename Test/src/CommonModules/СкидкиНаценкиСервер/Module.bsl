...
Функция ПрименитьРассчитанныеСкидкиКОбъекту(Объект, РасчетныеСкидки, НаименованиеТЧ = "Запасы") Экспорт
	...
	Если РасчетныеСкидки.Свойство("ТаблицаПодарков", ТаблицаПодарков) Тогда
		Для Каждого СтрокаТЧ Из Объект[НаименованиеТЧ] Цикл
		...
		КонецЕсли;
		ОписаниеРанееВыданныхПодарков = УчестьРанееВыданныеПодарки(Объект[НаименованиеТЧ], ТаблицаПодарков, ТЧПодарки);
...
Функция СтруктураРассчитанныхСкидок(ДеревоСкидок, Параметры)
	...
	Если Параметры.РассчитыватьБонусы Тогда
		...
		ВиртуальныйОбъект.Вставить("Подарки", ТаблицаПодарков.СкопироватьКолонки());
		...
		Если НЕ (Параметры.Свойство("НачислятьБонусыДоСкидок") И Параметры.НачислятьБонусыДоСкидок) Тогда
			...
			ДанныеТолькоСкидки = Новый Структура("ДеревоСкидок, ТаблицаСкидкиНаценки", ВозвращаемыеДанные.ДеревоСкидок, ВозвращаемыеДанные.ТаблицаСкидкиНаценки);
		...
		РассчитатьОкругления(ДеревоСкидок, Параметры, ТаблицаДанныхРасчетаБонусов, Истина);
...
Функция УчестьРанееВыданныеПодарки(ТабличнаяЧасть, ТаблицаПодарковКВыдаче, ВыданныеПодаркиНеВТоварах = Неопределено, ЭтоРМК = Ложь) Экспорт
	...
	ПодаркиВТоварах = Товары.Скопировать(СтрокиПодарковВТоварах, СписокКолонок);
	ПодаркиВТоварах.Индексы.Добавить("Номенклатура, Характеристика, Упаковка, Подарок");
	ПодаркиВТоварах.Индексы.Добавить("КлючСвязи");
	...
	Если ТаблицаПодарковКВыдаче.Количество() > 0 Тогда
		...
		Для Каждого ПодарокКВыдаче Из ТаблицаПодарковКВыдаче Цикл
			...
			Если ТипЗнч(ПодарокКВыдаче.ЕдиницаИзмерения) = Тип("СправочникСсылка.ЕдиницыИзмерения") Тогда
				КоэффициентУпаковки = ПодарокКВыдаче.ЕдиницаИзмерения.Коэффициент;
			КонецЕсли;
			ОтборРанееВыданныхПодарков = Новый Структура("Номенклатура, Характеристика, Упаковка, Подарок", 
				ПодарокКВыдаче.Номенклатура, 
				ПодарокКВыдаче.Характеристика, 
				ПодарокКВыдаче.ЕдиницаИзмерения,
				Истина);
			...
			ОтборРанееВыданныхПодарков.Упаковка = ТипЕдиницИзмерения.ПривестиЗначение(ОтборРанееВыданныхПодарков.Упаковка);
			Если Не ЗначениеЗаполнено(ОтборРанееВыданныхПодарков.Упаковка) Тогда
				ОтборРанееВыданныхПодарков.Удалить("Упаковка");
			КонецЕсли;
			РанееВыданныеПодарки = ПодаркиВТоварах.НайтиСтроки(ОтборРанееВыданныхПодарков);
			Для Каждого СтрокаТоваровРанееВыданногоПодарка Из РанееВыданныеПодарки Цикл				
				Если КоличествоПодарковВыдать <= 0 Тогда
					Прервать;
				КонецЕсли;
				ЗафиксированныйРанееПодарок = ПодаркиУжеВыданы.Добавить();
				ЗаполнитьЗначенияСвойств(ЗафиксированныйРанееПодарок, ПодарокКВыдаче,,"ПодарокИзСписка, Количество, Сумма");
				ЗафиксированныйРанееПодарок.СтрокаТоваров = СтрокаТоваровРанееВыданногоПодарка;
				
				КоличествоУпаковокПодарковВыдано = Мин(СтрокаТоваровРанееВыданногоПодарка.КоличествоУпаковок, КоличествоПодарковВыдать);
				НовоеКоличествоУпаковок = Макс(СтрокаТоваровРанееВыданногоПодарка.КоличествоУпаковок - КоличествоУпаковокПодарковВыдано, 0);
				НовоеКоличество = НовоеКоличествоУпаковок * КоэффициентУпаковки;
				СуммаВыдано = 0;
				Если НЕ ПодарокКВыдаче.Количество = 0 Тогда
					СуммаВыдано = Окр(ПодарокКВыдаче.Сумма * (КоличествоУпаковокПодарковВыдано / ПодарокКВыдаче.Количество), 2);
				КонецЕсли;
				НоваяСумма = Макс(СтрокаТоваровРанееВыданногоПодарка.Сумма - СуммаВыдано, 0);
				СтрокаТоваровРанееВыданногоПодарка.КоличествоУпаковок = НовоеКоличествоУпаковок;
				СтрокаТоваровРанееВыданногоПодарка.Количество = НовоеКоличество;
				СтрокаТоваровРанееВыданногоПодарка.Сумма = НоваяСумма;
				
				ЗафиксированныйРанееПодарок.Количество = КоличествоУпаковокПодарковВыдано;
				ЗафиксированныйРанееПодарок.Сумма = СуммаВыдано; 
			...
			Если ИскатьВТЧПодарков И КоличествоПодарковВыдать > 0 Тогда
				РанееВыданныеПодарки = ПодаркиВТаблицеВыданныхПодарков.НайтиСтроки(ОтборРанееВыданныхПодарков);
				Для Каждого СтрокаТаблицыРанееВыданногоПодарка Из РанееВыданныеПодарки Цикл				
					Если КоличествоПодарковВыдать <= 0 Тогда
						Прервать;
					КонецЕсли;
					КоличествоУпаковокПодарковВыдано = Мин(СтрокаТаблицыРанееВыданногоПодарка.Количество, КоличествоПодарковВыдать);
					НовоеКоличество = Макс(СтрокаТаблицыРанееВыданногоПодарка.Количество - КоличествоУпаковокПодарковВыдано, 0);
					СуммаВыдано = 0;
					Если НЕ ПодарокКВыдаче.Количество = 0 Тогда
						СуммаВыдано = Окр(ПодарокКВыдаче.Сумма * (КоличествоУпаковокПодарковВыдано / ПодарокКВыдаче.Количество), 2);
					КонецЕсли;
					НоваяСумма = Макс(СтрокаТаблицыРанееВыданногоПодарка.Сумма - СуммаВыдано, 0);
					СтрокаТаблицыРанееВыданногоПодарка.Количество = НовоеКоличество;				
					СтрокаТаблицыРанееВыданногоПодарка.Сумма = НоваяСумма;				
			...
			Если КоличествоПодарковВыдано > 0 Тогда				
				...
				Для Каждого ДругойВариант Из ВариантыПодарка Цикл
					...
					Если ТипЗнч(ДругойВариант.ЕдиницаИзмерения) = Тип("СправочникСсылка.ЕдиницыИзмерения") Тогда
						КоэффициентУпаковкиВарианта = ДругойВариант.ЕдиницаИзмерения.Коэффициент;
					КонецЕсли;					
					НовоеКоличество = Цел(ДругойВариант.Количество * КоэффициентУпаковкиВарианта - КоличествоПодарковВыдано * КоэффициентУпаковки) / КоэффициентУпаковкиВарианта;
					НоваяСумма = Окр(ДругойВариант.Сумма * (НовоеКоличество / ДругойВариант.Количество), 2);
	...
	Для Каждого СтрокаВыданногоРанееПодарка Из ПодаркиВТоварах Цикл
		Отбор = Новый Структура();
		Если ЭтоРМК Тогда
		...
		Для Каждого СтрокаПодаркаДокумента Из СтрокиПодарков Цикл
			...
			Если ЭтоРМК Тогда
				НовоеКоличество = СтрокаПодаркаДокумента.Количество - СтрокаВыданногоРанееПодарка.Количество;	
				НовоеКоличествоУпаковок = СтрокаПодаркаДокумента.КоличествоУпаковок - СтрокаВыданногоРанееПодарка.КоличествоУпаковок;	
				СтрокаПодаркаДокумента.Количество = НовоеКоличество;	
				СтрокаПодаркаДокумента.КоличествоУпаковок = НовоеКоличествоУпаковок;	
			Иначе
				НовоеКоличествоУпаковок = СтрокаПодаркаДокумента.Количество - СтрокаВыданногоРанееПодарка.КоличествоУпаковок;	
				СтрокаПодаркаДокумента.Количество = НовоеКоличествоУпаковок;	
			КонецЕсли;
			НоваяСумма = СтрокаПодаркаДокумента.Сумма - СтрокаВыданногоРанееПодарка.Сумма;	
