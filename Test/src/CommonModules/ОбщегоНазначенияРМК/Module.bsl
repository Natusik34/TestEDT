...
Процедура НастроитьВидимостьЭлементовНастройкиРМК(Форма) Экспорт
	...
	Элементы.ГруппаБонусыИСертификаты.ОтображениеПодсказки
		= ?(Форма.ВводДисконтныхКарт, ОтображениеПодсказки.ОтображатьСверху, ОтображениеПодсказки.ОтображатьСлева);
	Элементы.ГруппаБонусыИСертификаты.Отображение
		= ?(Форма.ВводДисконтныхКарт, ОтображениеОбычнойГруппы.Нет, ОтображениеОбычнойГруппы.ОбычноеВыделение);
	Элементы.ГруппаБонусыИСертификатыРасширеннаяПодсказка.ВертикальноеПоложение
		= ?(Форма.ВводДисконтныхКарт, ВертикальноеПоложениеЭлемента.Центр, ВертикальноеПоложениеЭлемента.Верх);
...
Процедура РассчитатьАвтоматическиеСкидки(ФормаРМК) Экспорт
	...
	// Применение подарков влияет на скидки - скидка привязывается к строке подарка, поэтому загрузка табли связана
	...
	Если ДанныеОтвета.Свойство("MaxBonusPay") Тогда
...
Функция ПолучитьСтруктуруПараметровРасчетаСкидок(ФормаРМК)
	...
	СтруктураПараметров.Вставить("GetMaxBonusPay", НеобходимоЗапроситьСуммуМаксимальнойОплатыБаллами(ФормаРМК));
...
Функция НеобходимоЗапроситьСуммуМаксимальнойОплатыБаллами(ФормаРМК)
	
	РезультатФункции = Ложь;
	
	Если ЗначениеЗаполнено(ФормаРМК.Объект.ДисконтнаяКарта) Тогда
...
Процедура ПолучитьДанныеДляСверки(ПараметрыПроцедуры, АдресРезультата) Экспорт
	...
	СтатусКассовойСмены = КассовыеСмены.СтатусПоследнейСмены(УстройствоФискализации);
	
	ДанныеСверки = ОФД.ОборотыКассовойСмены(ПараметрыПроцедуры.КассаККМ, СтатусКассовойСмены.НомерСменыККТ);
	...
	Если Не Отказ Тогда
		ПоместитьВоВременноеХранилище(ДанныеСверки.ДанныеСмены, АдресРезультата);
	КонецЕсли;
