...
Функция ДанныеОрганизаций(ПараметрыОбмена, МассивОтбора)
	...
	РуководительИдентификатор = СсылкаПубличныйИдентификатор(ТаблицаДанных, "Руководитель", Перечисления.ТипыОбъектовИнтеграцияУправлениеПерсоналом.ФизическоеЛицо);
	ДолжностьИдентификатор = СсылкаПубличныйИдентификатор(ТаблицаДанных, "Должность", Перечисления.ТипыОбъектовИнтеграцияУправлениеПерсоналом.Должность);
...
Функция ДанныеМашиночитаемыхДоверенностейКЭДО(ПараметрыОбмена, МассивОтбора)
	...
	ТаблицаДанных.Колонки.Добавить("МЧДИдентификатор", Новый ОписаниеТипов("Строка"));
	ПубличныеИдентификаторы = СсылкаПубличныйИдентификатор(ТаблицаДанных, "МЧД", Перечисления.ТипыОбъектовИнтеграцияУправлениеПерсоналом.МашиночитаемаяДоверенностьКЭДО);
	
	Для каждого СтрокаТЗ Из ТаблицаДанных Цикл
		СтрокаТЗ.МЧДИдентификатор = ПубличныеИдентификаторы[СтрокаТЗ.МЧД];
	КонецЦикла;
	
	ТаблицаДанных.Колонки.Удалить("МЧД");
	ТаблицаДанных.Колонки.МЧДИдентификатор.Имя = "МЧД";
...
Функция ДанныеРасчетныхЛистовДляПубликацииВариантовПоВарианту(Организация, Месяц, СписокФизическихЛиц, ПерваяПоловинаМесяца, ВариантОтчета = Неопределено)
	...
	Возврат ЗарплатаКадрыОтчеты.ДанныеРасчетныхЛистков(
		СписокФизическихЛиц, Организация, НачалоМесяца(Месяц), КонецМесяца(Месяц), ОтчетРасчетныйЛисток);
КонецФункции
...
Функция РезультатВыгрузкиМашиночитаемыхДоверенностейКЭДО(ПараметрыОбмена, Список)
	...
	РазмерПакета = РазмерПакета(ПараметрыОбмена, Перечисления.ТипыОбъектовИнтеграцияУправлениеПерсоналом.МашиночитаемаяДоверенностьКЭДО);
	РезультатВыгрузки = РезультатВыгрузкиКоллекцииВСервис(ПараметрыОбмена, РесурсСервиса, Данные, "ID", РазмерПакета);
...
Функция ИзмененияДляПубликации()
	...
	ТипыОбъектов.Добавить(Перечисления.ТипыОбъектовИнтеграцияУправлениеПерсоналом.МашиночитаемаяДоверенностьКЭДО);
...
Процедура ПроверитьВыгрузитьДанныеРуководителей(ПараметрыОбмена, РезультатВыгрузки)
	...
	Если ЗначениеЗаполнено(РуководителиОрганизаций) Тогда
		...
		Если ЗначениеЗаполнено(ФизическиеЛицаДляВыгрузки) Тогда
			РезультатВыгрузкиРуководителей = РезультатВыгрузкиФизическихЛиц(ПараметрыОбмена, ФизическиеЛицаДляВыгрузки);
