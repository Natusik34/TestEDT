...
Процедура НачатьВызовШиринаСтроки(ОповещениеПриЗавершении, ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Исключение   
		Если ОповещениеПриЗавершении <> Неопределено Тогда
			РезультатВыполнения = ОшибкаВызоваДрайвера("ПолучитьШиринуСтроки", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ВыполнитьОбработкуОповещения(ОповещениеПриЗавершении, РезультатВыполнения);
		КонецЕсли;
	КонецПопытки;
...
Процедура НачатьВызовПолучитьШиринуСтрокиЗавершение(РезультатВызова, ПараметрыВызова, Контекст)
	...
	ИначеЕсли ПараметрыПодключения.ТипыОборудования.ККТ Тогда
		...
		Если ПараметрыПодключения.РевизияИнтерфейса >= 3004  Тогда
			...
			Если НЕ ПустаяСтрока(ТекущийСтатус.ИдентификаторЗапроса) Тогда
				Попытка
					РезультатВызова = ОбъектДрайвера.ПодтвердитьКМ(ПараметрыПодключения.ИДУстройства, ТекущийСтатус.ИдентификаторЗапроса, 0);
		...
		Попытка
			Если ПараметрыПодключения.РевизияИнтерфейса >= 3000 Тогда // Версия стандарта 3.0 (ОФД 1.1)
				...
				КонецЕсли;
				РезультатВызова = ОбъектДрайвера.СформироватьЧек(ПараметрыПодключения.ИДУстройства, ДанныеОперации.Электронно, ДанныеЧекаXML, ДанныеОперации.РезультатОперацииXML); 
...
Функция ОткрытьСмену(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	ИначеЕсли ПараметрыПодключения.ТипыОборудования.ККТ Тогда        
		...
		ПараметрыККТ = "";
		Попытка          
			РезультатВызова = ОбъектДрайвера.ПолучитьПараметрыККТ(ПараметрыПодключения.ИДУстройства, ПараметрыККТ);
		Исключение
...
Функция ОтчетОТекущемСостоянииРасчетов(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Если ПараметрыПодключения.ТипыОборудования.ККТ Тогда
		...
		РезультатXML = "";
		Попытка
			РезультатВызова = ОбъектДрайвера.ОтчетОТекущемСостоянииРасчетов(ПараметрыПодключения.ИДУстройства, ДанныеОперации.ПараметрыXML, РезультатXML);
			Если РезультатВызова.Значение Тогда   
				РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);   
...
Функция ПараметрыККТ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации);
	...
	Если ПараметрыПодключения.ТипыОборудования.ККТ Тогда
		...
		Попытка
			РезультатВызова = ОбъектДрайвера.ПолучитьПараметрыККТ(ПараметрыПодключения.ИДУстройства, "");
		Исключение
...
Функция ТекущееСостояние(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		Если ПараметрыПодключения.РевизияИнтерфейса >= 3000 Тогда // Версия стандарта 3.0 (ОФД 1.1)
			РезультатВызова = ОбъектДрайвера.ПолучитьТекущееСостояние(ПараметрыПодключения.ИДУстройства, ДанныеОперации.ПараметрыXML, ДанныеОперации.РезультатXML);
		Иначе
...
Функция РезультатыЗапросаКМ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		РезультатВызова = ОбъектДрайвера.ПолучитьРезультатыЗапросаКМ(ПараметрыПодключения.ИДУстройства, РезультатОперацииXML, СтатусЗапроса);
		Если РезультатВызова.Значение Тогда                   
			РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);  
...
// Процедура возвращает параметры терминале.
//
Процедура НачатьВызовПараметрыТерминала_Завершение(РезультатВызова, ПараметрыВызова, Контекст)
...
Процедура НачатьВызовПараметрыТерминала_ПечатьКвитанцийНаТерминалеЗавершение(РезультатВызова, ПараметрыВызова, Контекст)
		...
		Попытка                  
			Если РевизияИнтерфейса >= 4000 Тогда    
				...
				Если Не ПустаяСтрока(РеквизитыКартыQR) И НЕ ПараметрыПодключения.ConsumerPresentedQR Тогда
					...
					Возврат РезультатВыполнения;   
				КонецЕсли;
				РезультатВызова = ОбъектДрайвера.ПолучитьПараметрыКарты(ПараметрыПодключения.ИДУстройства, РеквизитыКартыQR, ОтПоследнейОперации, НомерКарты, ХешНомерКарты, СсылкаНаПлатежныйСчет, ТипКарты, СвояКарта);
			ИначеЕсли РевизияИнтерфейса >= 3007 Тогда
				РезультатВызова = ОбъектДрайвера.ПолучитьПараметрыКарты(ПараметрыПодключения.ИДУстройства, ОтПоследнейОперации, НомерКарты, ХешНомерКарты, СсылкаНаПлатежныйСчет, ТипКарты, СвояКарта);
			Иначе
				РезультатВызова = ОбъектДрайвера.ПолучитьПараметрыКарты(ПараметрыПодключения.ИДУстройства, ОтПоследнейОперации, НомерКарты, ХешНомерКарты, ТипКарты, СвояКарта);
			КонецЕсли;
			Если РезультатВызова.Значение Тогда     
				РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);  
				Если РевизияИнтерфейса >= 4000 Тогда   
...
Функция ОперацииПоКартам(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Иначе               
		...
		КонецЕсли;
		Попытка
			РезультатВызова = ОбъектДрайвера.ПолучитьОперацииПоКартам(ПараметрыПодключения.ИДУстройства, "");  
			Если РезультатВызова.Значение Тогда        
				РезультатОперацииXML = РезультатВызова.Параметры[1];
				РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);   
...
Функция ИтогиДняПоКартам(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	ТекстСлипЧека = "";
	Попытка            
		РезультатВызова = ОбъектДрайвера.ИтогиДняПоКартам(ПараметрыПодключения.ИДУстройства, ТекстСлипЧека);
		Если РезультатВызова.Значение Тогда                    
			РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);  
...
Функция ПараметрыВывода(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Если ПараметрыПодключения.РевизияИнтерфейса >= 3004 Тогда
		Попытка
			РезультатВызова = ОбъектДрайвера.ПолучитьПараметрыДисплеяПокупателя(ПараметрыПодключения.ИДУстройства, "");   
			РезультатОперацииXML = РезультатВызова.Параметры[1];
			Если РезультатВызова.Значение Тогда
				ПараметрыТерминала = ОбщегоНазначенияБПОКлиент.ПрочитатьКорневойЭлементXML(РезультатОперацииXML);
				Если ПараметрыТерминала.Свойство("Columns") И Не ПустаяСтрока(ПараметрыТерминала.Columns) Тогда
	...
	Иначе
		Попытка
			РезультатВызова = ОбъектДрайвера.ПолучитьПараметрыВывода(ПараметрыПодключения.ИДУстройства, 20, 2); 
			Если РезультатВызова.Значение Тогда
...
Функция ПолучитьВес(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		РезультатВызова = ОбъектДрайвера.ПолучитьВес(ПараметрыПодключения.ИДУстройства, Вес);
		Если РезультатВызова.Значение Тогда                    
...
Функция ЗагрузитьТаблицу(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка                     
		ДанныеЗагрузки = ""; 
		РезультатВызова = ОбъектДрайвера.ЗагрузитьТаблицу(ПараметрыПодключения.ИДУстройства, ДанныеЗагрузки);
		Если РезультатВызова.Значение Тогда                    
			РезультатВыполнения = РезультатОперацииНаОборудовании(Истина); 
...
Функция ПолучитьДанныеМетокRFID(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации) 
	...
	Попытка       
		РезультатВызова = ОбъектДрайвера.ПолучитьДанныеМетокRFID(ПараметрыПодключения.ИДУстройства, ИДПакета, ТаблицаМеток);
		Если РезультатВызова.Значение Тогда                    
			РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);   
