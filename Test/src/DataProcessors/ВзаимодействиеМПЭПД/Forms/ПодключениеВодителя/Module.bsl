...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПредставленияРолейМП = СервисВзаимодействияМПЭПДКлиентСервер.ПредставленияРолейМП(Ложь);
...
Процедура ЗаполнитьСписокДоверенностей()
	...
	|	И МашиночитаемыеДоверенностиОрганизаций.ДоверительФЛ_ИНН = &ПредставительИНН
	...
	Запрос.УстановитьПараметр("СтатусВРеестреФНС", ВсеСтатусыДоверенности());
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ДоверенностьДействительна(Выборка, ТекущаяДатаСеанса()) Тогда
	...
	Если СписокВыбора.Количество() > 0 Тогда
		СписокВыбора.Добавить(Справочники.МашиночитаемыеДоверенностиОрганизаций.ПустаяСсылка(), "Отозвать доверенность");
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Функция ДоверенностьДействительна(СвойстваДоверенности, ДатаПроверки)
	...
	ДействительнаПоСтатусу = ВсеСтатусыДоверенности().Найти(СвойстваДоверенности.СтатусВРеестреФНС) <> Неопределено;
...
&НаСервере
Функция ВсеСтатусыДоверенности()
