...
Функция РезультатВыгрузкиОбъектов(ТаблицаДанных, ТипОбъекта) Экспорт
	...
	Если ТипОбъекта = Перечисления.ТипыОбъектовИнтеграцияУправлениеПерсоналом.ФизическоеЛицо Тогда
		ВыгрузитьФотографии(ТаблицаДанных);
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ТипОбъектаКлюча) Тогда
		БылиОшибки = Истина;
		ЗаписатьОшибкуНеЗаданТипКлюча(ТипОбъекта);
		Возврат Результат;
	КонецЕсли;
...
Процедура ЗаписатьОшибкуНеЗаданТипКлюча(ТипОбъекта)
	
	ШаблонОписания = НСтр("ru = 'Ошибка обмена. Не задан тип ключа для объекта: %1.'");
	Комментарий = СтрШаблон(ШаблонОписания, Строка(ТипОбъекта));
	ЗаписьЖурналаРегистрации(ИменаСобытийЖР.Обмен, УровеньЖурналаРегистрации.Ошибка,,,Комментарий);
