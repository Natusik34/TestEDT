...
Функция МаркируемаяПродукция()
	
	МаркируемаяПродукция = Новый ТаблицаЗначений();
	МаркируемаяПродукция.Колонки.Добавить("Номенклатура",                    Метаданные.ОпределяемыеТипы.Номенклатура.Тип);
	МаркируемаяПродукция.Колонки.Добавить("Характеристика",                  Метаданные.ОпределяемыеТипы.ХарактеристикаНоменклатуры.Тип);
	МаркируемаяПродукция.Колонки.Добавить("Серия",                           Метаданные.ОпределяемыеТипы.СерияНоменклатуры.Тип);
	МаркируемаяПродукция.Колонки.Добавить("Количество",                      Новый ОписаниеТипов("Число"));
	МаркируемаяПродукция.Колонки.Добавить("GTIN",                            Метаданные.ОпределяемыеТипы.GTIN.Тип);
	МаркируемаяПродукция.Колонки.Добавить("Идентификатор",                   Новый ОписаниеТипов("Число"));
	МаркируемаяПродукция.Колонки.Добавить("ИдентификаторПроисхожденияВЕТИС", Метаданные.ОпределяемыеТипы.ИдентификаторПроисхожденияВЕТИС.Тип);
	МаркируемаяПродукция.Колонки.Добавить("ГоденДо",                         Новый ОписаниеТипов("Дата"));
	
	Возврат МаркируемаяПродукция;
...
Процедура ДополнитьКолонкамиРозничнойПродажи(Таблица)
	...
	Таблица.Колонки.Добавить("ИдентификаторЗапросаГИСМТ",        Метаданные.ОпределяемыеТипы.УникальныйИдентификаторИС.Тип);
	Таблица.Колонки.Добавить("ДатаВремяПолученияИдентификатора", Новый ОписаниеТипов("Дата",,, Новый КвалификаторыДаты(ЧастиДаты.ДатаВремя)));
	Таблица.Колонки.Добавить("ДанныеЗапросаГИСМТ",               Новый ОписаниеТипов("ХранилищеЗначения"));
...
Процедура ЗаполнитьШтрихкодыВерхнегоУровняИдентификаторамиЗапросовГИСМТ(СтрокиДерева, ТаблицаШтрихкодов)
	...
	Для Каждого Строка Из СтрокиДерева Цикл
		...
		Для Каждого СтрокаТаблицы Из СтрокиТаблицы Цикл
			...
			СтрокаТаблицы.ИдентификаторЗапросаГИСМТ        = Строка.ИдентификаторЗапросаГИСМТ;
			СтрокаТаблицы.ДатаВремяПолученияИдентификатора = Строка.ДатаВремяПолученияИдентификатора;
			
			СтруктураДанныхИдентификатораГИСМТ = Новый Структура("ИдентификаторЗапросаГИСМТ, ДатаВремяПолученияИдентификатора", "", Дата(1,1,1));
			ЗаполнитьЗначенияСвойств(СтруктураДанныхИдентификатораГИСМТ, СтрокаТаблицы);
			
			ХранилищеДанныхИдентификатора = Новый ХранилищеЗначения(СтруктураДанныхИдентификатораГИСМТ);
			СтрокаТаблицы.ДанныеЗапросаГИСМТ = ХранилищеДанныхИдентификатора;
