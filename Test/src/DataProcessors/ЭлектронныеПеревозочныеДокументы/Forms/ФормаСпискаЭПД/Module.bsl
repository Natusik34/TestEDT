...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.СписокОткрытьНастройкиДопРеквизитов.Видимость = ОбменСГИСЭПДКлиентСервер.ИспользуютсяДополнительныеРеквизитыЭПД();
...
Процедура ОбновитьИспользованиеМобильногоПриложения(НовоеЗначение = Неопределено)
	...
	Если НовоеЗначение = Неопределено Тогда
		...
		Если ИспользованиеМобильногоПриложенияНастройка = Неопределено Тогда
			СписокПодключенныхМП = СервисВзаимодействияМПЭПД.СписокПодключенныхМП();
...
&НаКлиенте
Процедура ИспользуетсяМобильноеПриложениеПриИзменении(Элемент)
...
Процедура НастройкаВзаимодействияМП(Команда)
	...
	ОткрытьФорму("Обработка.ВзаимодействиеМПЭПД.Форма.СостояниеПодключенияМП",
				ПараметрыФормы,
				ЭтотОбъект,
...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.МобильноеПриложение Тогда
		ДанныеСтроки = Элементы.Список.ДанныеСтроки(ВыбраннаяСтрока);
		Если ДанныеСтроки.Организация <> ДанныеСтроки.Перевозчик Тогда
			ПоказатьПредупреждение(, НСтр("ru='Мобильное приложение доступно только перевозчику'"));
		Иначе
			ПараметрыДействийМП = СервисВзаимодействияМПЭПДКлиент.ПараметрыДействийДляМобильногоПриложения(
				ДанныеСтроки.Ссылка,
			...
			Если ЗначениеЗаполнено(ДанныеСтроки.МобильноеПриложение) Тогда			
				...
				КонецЕсли;
				ОписаниеОповещения = Новый ОписаниеОповещения("ВыборДействияМП_Завершение", ЭтотОбъект, ПараметрыДействийМП);
				ПоказатьВыборИзМеню(ОписаниеОповещения, СписокМеню, Поле);
			Иначе
				ВыборДействияМП_Завершение(Новый Структура("Значение", "Выбрать"), ПараметрыДействийМП);	
			КонецЕсли;	
		КонецЕсли;
		Возврат;	
	КонецЕсли; 
...
&НаКлиенте
Процедура ВыборДействияМП_Завершение(Результат, ПараметрыДействийМП) Экспорт
	...
	ИначеЕсли Результат.Значение = "Отвязать" Тогда
		Результат = СервисОтвязатьДокумент(ПараметрыДействийМП.Документ, ПараметрыДействийМП.ИдентификаторЭДО);
...
&НаСервере
Функция СервисОтвязатьДокумент(Документ, ИдентификаторЭДО)
...
Процедура УстановитьОтборыДинамическогоСписка()
	...
	Список.Параметры.УстановитьЗначениеПараметра("МассивШаговМПТребующихРеакцию", МассивШаговМПТребующихРеакцию);
	Список.Параметры.УстановитьЗначениеПараметра("МассивШаговМПТребующихРеакциюОтклонение", МассивШаговМПТребующихРеакциюОтклонение);
