...
Процедура ИмяФайлаПравилПриИзмененииЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		НачатьПомещениеФайлаНаСервер(ОписаниеОповещения,,, АдресФайлаПравилВХранилище, ИмяФайлаПравил, УникальныйИдентификатор);
...
Процедура ПолучитьИнформациюОФайлеОбмена(Команда)
	...
	НачатьПомещениеФайлаНаСервер(ОписаниеОповещения,,,АдресФайла, ИмяФайлаОбмена, УникальныйИдентификатор);
...
Процедура ПолучитьИнформациюОФайлеОбменаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Попытка
		...
		ОткрытьФайлЗагрузкиНаСервере(Результат.Адрес);
		ПредставлениеПериодаВыгрузки = ПредставлениеПериода(Объект.ДатаНачала, Объект.ДатаОкончания);
...
Процедура НастройкаОтладкиВыгрузки(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПрочитатьПравилаОбменаДляОтладкиЗавершение", ЭтотОбъект);
	НачатьПомещениеФайлаНаСервер(ОписаниеОповещения,,, АдресФайлаПравилВХранилище, ИмяФайлаПравил, УникальныйИдентификатор);
...
Процедура НастройкаОтладкиЗагрузки(Команда)
	...
	НачатьПомещениеФайлаНаСервер(ОписаниеОповещения,,, АдресФайлаОбменаВХранилище, ИмяФайлаОбмена, УникальныйИдентификатор);
...
Процедура НастройкаОтладкиЗагрузкиЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Объект.ИмяФайлаОбмена = ИмяФайлаНаСервереИлиКлиенте(ИмяФайлаОбмена, Результат.Адрес);
...
Процедура ВыполнитьВыгрузку(Команда)
	...
	Если Не ЗначениеЗаполнено(ИмяФайлаДанных) И Не ПрямаяВыгрузка Тогда
		СообщитьПользователю(НСтр("ru = 'Не указано имя файла данных.'"), "ИмяФайлаДанных");
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьЗагрузку(Команда)
	...
	Если Не ЗначениеЗаполнено(ИмяФайлаОбмена) Тогда
		СообщитьПользователю(НСтр("ru = 'Не указано имя файла для загрузки.'"), "ИмяФайлаОбмена");
		Возврат;
	КонецЕсли;
...
Процедура ПрочитатьПравилаОбмена(Команда)
	...
	Если Не ЗначениеЗаполнено(ИмяФайлаПравил) Тогда
		СообщитьПользователю(НСтр("ru = 'Не указан файл правил для загрузки данных.'"), "ИмяФайлаПравил");
		Возврат;
	КонецЕсли;
...
Процедура ПрочитатьПравилаОбменаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ВыполнитьЗагрузкуПравилОбмена(Результат.Адрес, Результат.СсылкаНаФайл.Имя);
...
&НаКлиенте
Процедура ПрочитатьПравилаОбменаДляОтладкиЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
Процедура ОткрытьФайлЗагрузкиНаСервере(АдресФайла)
	...
	Объект.ИмяФайлаОбмена = ИмяВременногоФайла;
...
Процедура ВыполнитьЗагрузкуИзФормыЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ВыполнитьЗагрузкуНаСервере(Результат.Адрес, Результат.СсылкаНаФайл.Имя);
...
Процедура ВыполнитьВыгрузкуИзФормы()
	...
	Если Не ПрямаяВыгрузка И Не Объект.ФлагОшибки Тогда
		НачатьПолучениеФайлаССервера(,АдресФайлаДанныхВХранилище, ИмяФайлаДанных)
	КонецЕсли;
...
Процедура ИзменитьРежимБезопаснаяЗагрузка(Интерактивно = Истина)
	...
	Если Объект.БезопаснаяЗагрузка И ЗначениеЗаполнено(ИмяФайлаПравилЗагрузки) Тогда
		ПоместитьФайлПравилЗагрузкиВХранилище();
	КонецЕсли;
