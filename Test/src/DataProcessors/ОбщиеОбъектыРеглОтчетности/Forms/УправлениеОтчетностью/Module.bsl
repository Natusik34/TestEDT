...
Процедура ПоказатьБланк(Команда)
	...
	Если Элементы.ЖурналОтчетов.ТекущиеДанные.НаименованиеОтчета = "Расчет доли доходов личного фонда" Тогда
		ПоказатьПредупреждение(, НСтр(
		"ru='Печать со штрихкодом PDF417 формы
		|отчета, утвержденной приказом ФНС России
		|от 06.10.2023 № ЕА-7-3/718@, будет
...
Процедура ПоказатьСДвухмернымШтрихкодомPDF417(Команда)
	...
	Если Элементы.ЖурналОтчетов.ТекущиеДанные.НаименованиеОтчета = "6-НДФЛ (с 2021 года)"
		И ОписаниеРегОтчета(Элементы.ЖурналОтчетов.ТекущаяСтрока).ВыбраннаяФорма = "ФормаОтчета2024Кв2" Тогда
		ПоказатьПредупреждение(, НСтр(
		"ru='Печать со штрихкодом PDF417 формы
