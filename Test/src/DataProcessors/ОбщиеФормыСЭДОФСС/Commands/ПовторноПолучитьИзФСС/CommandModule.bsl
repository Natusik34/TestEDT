...
Процедура ОбработкаКоманды(МассивСсылок, ПараметрыВыполненияКоманды)
	...
	КонецЕсли;
	СтандартныеПодсистемыКлиент.ОповеститьФормыОбИзменении(Результат.ОповещениеФорм);
КонецПроцедуры
...
Функция ПовторноПолучитьИзФССНаСервере(Знач МассивСсылок)
	...
	ОповещениеФорм = СтандартныеПодсистемыСервер.ПодготовитьОповещениеФормОбИзменении(МассивСсылок);
	
	Результат = Новый Структура("НовыеСообщения, ТекстыОшибок, Всего, ОповещениеФорм");
	Результат.НовыеСообщения = НовыеСообщения;
	Результат.ТекстыОшибок   = ТекстыОшибок;
	Результат.Всего          = Всего;
	Результат.ОповещениеФорм = ОповещениеФорм;
	Возврат Результат;
КонецФункции
