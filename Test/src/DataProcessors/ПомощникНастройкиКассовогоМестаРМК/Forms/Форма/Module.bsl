...
Процедура ПрименитьОтборПоТипуОплатыКСпискуВидовОплат(ИмяПоля = "ТипОплаты")
	...
	УстановитьОтбор = НЕ (ИспользоватьОплатуПлатежнымиКартами И НастройкаРМК.ИспользоватьОплатуПлатежнымиСистемами);
...
Процедура БольшеНастроек(Команда)
	МенеджерОборудованияРМККлиент.ВыполнитьНастройкуОборудования(ИдентификаторОборудования);
КонецПроцедуры
...
Процедура ВыборПроизводительАтол(Команда)
	...
	Для каждого СтрокаСписка Из СписокДрайверов Цикл
		Если СтрНайти(СтрокаСписка.Представление, НСтр("ru ='АТОЛ%'")) > 0 Тогда
...
Процедура ВыборПроизводительШтрихМ(Команда)
	...
	Для каждого СтрокаСписка Из СписокДрайверов Цикл
		Если СтрНайти(СтрокаСписка.Представление, НСтр("ru ='ШТРИХ-М%'")) > 0 Тогда
			Элементы.ДрайверОборудованияНастройка.СписокВыбора.Добавить(СтрокаСписка.Значение, СтрокаСписка.Представление);
		КонецЕсли;
	КонецЦикла;
...
Процедура ВыборПроизводительДримкас(Команда)
	...
	Для каждого СтрокаСписка Из СписокДрайверов Цикл
		Если СтрНайти(СтрокаСписка.Представление, НСтр("ru ='Дримкас%'")) > 0 Тогда
			Элементы.ДрайверОборудованияНастройка.СписокВыбора.Добавить(СтрокаСписка.Значение, СтрокаСписка.Представление);
		КонецЕсли;
	КонецЦикла;
...
Процедура СверятьДанныеССервисомОФДПриИзменении(Элемент)
	//Если СверятьДанныеССервисомОФД = ТекущаяКассаККМ.СверятьДанныеССервисомОФД Тогда
	//	НастройкиСверкиКассыККМИзменены = Ложь;
	//Иначе 
...
Процедура ДекорацияСверятьДанныеССервисомОФДНажатие(Элемент)
	...
	МодульОФД.ПараметрыПодключения(НастройкиПодключения, Неопределено);КонецПроцедуры
