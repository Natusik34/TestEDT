...
Процедура ВыполнитьРасчетНажатие()	
	...
	СособяниеСобытия = КалендарьОтчетности.ПолучитьСостояниеСобытияКалендаря(Организация, СобытиеКалендаря);
	Если СособяниеСобытия = ПредопределенноеЗначение("Перечисление.СостоянияСобытийКалендаря.Заполнить") Тогда
		//произошла ошибка блокировки при записи расчета
		Сообщение.Сообщить();
		Возврат;		
	КонецЕсли;
