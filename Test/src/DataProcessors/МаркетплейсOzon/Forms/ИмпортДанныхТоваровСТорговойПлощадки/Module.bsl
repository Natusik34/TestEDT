...
Процедура ПриОткрытии(Отказ)
	...
	Иначе
		ВариантЗагрузкиДанныхЗагрузитьОтсутствующиеКарточкиТоваров = 2;
		Если ЭтотОбъект.ВариантЗагрузкиДанных = ВариантЗагрузкиДанныхЗагрузитьОтсутствующиеКарточкиТоваров Тогда
...
Процедура ИмпортируемыеТоварыНоменклатураПриИзменении(Элемент)
	...
	НоменклатураЗначенияРеквизитов = ЗначенияРеквизитовОбъекта(ТекущиеДанные.Номенклатура, "ЕдиницаИзмерения, ИспользоватьХарактеристики" );
...
Процедура ИмпортируемыеТоварыНоменклатураСоздание(Элемент, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.Категория) Тогда
		...
		Если ИсточникКатегории = ПредопределенноеЗначение("Перечисление.ИсточникиКатегорийДляМаркетплейса.ВидНоменклатуры") Тогда
			...
			ДополнительныеПараметры.Вставить("ТипНоменклатуры", ЗначениеРеквизитаОбъекта(ТекущиеДанные.Категория, "ТипНоменклатуры"));
...
Процедура ДополнительныеПараметрыИмпортаДанных(Команда)
	...
	ОткрытьФорму("Обработка.МаркетплейсOzon.Форма.ИмпортДанныхНастройка", ПараметрыОткрытия, ЭтотОбъект,,,, ОповещениеОЗакрытии, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура УстановитьВидимостьДоступность()
	...
	Если ВариантЗагрузкиДанных = 0 Тогда
		ДополнительныеПараметрыЗаголовок = НСтр("ru = 'Обновить сопоставленные карточки товаров'");
	ИначеЕсли ВариантЗагрузкиДанных = 1 Тогда
		ДополнительныеПараметрыЗаголовок = НСтр("ru = 'Обновить сопоставленные карточки товаров и не сопоставленные карточки товаров'");
	Иначе
		ДополнительныеПараметрыЗаголовок = НСтр("ru = 'Загрузить не сопоставленные карточки товаров'");
	КонецЕсли;
	...
	Элементы.ИмпортируемыеТоварыУпаковка.Видимость           = Истина
...
Функция ЗаписатьЗагруженныеДанныеНаСервере(Знач ЗаписатьТолькоЦены = Ложь, Знач ТолькоСоздатьНоменклатуру = Ложь)
	...
	ПараметрыФункции.Вставить("ОтображатьСопоставленные",       ОтображатьСопоставленные);
	ПараметрыФункции.Вставить("ОтображатьПоСтатусу",            ОтображатьПоСтатусу);
	ПараметрыФункции.Вставить("ДокументУстановкаЦенНоменклатуры", ДокументУстановкаЦенНоменклатуры);
